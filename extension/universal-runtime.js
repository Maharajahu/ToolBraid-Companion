...
export const UI_MESSAGE_TYPES = Object.freeze({
  ...
  UI_EXECUTE_OWNER_ACTION: 'toolbraid.ui.execute-owner-action',
...
function describeDirectAction(tool, ownerMode) {
  if (!ownerMode || !isXDirectAction(tool)) return tool;
  return Object.freeze({
    ...tool,
    directExecution: true,
    description: `${tool.description} Uses enabled X direct control; no per-action ToolBraid confirmation is required.`,
  });
}

function uiState(tab, state, {
  audit = null,
  capture = null,
  multimodalProvider = null,
  ownerMode = false,
  fileInputs = [],
  extractorPageFingerprint = null,
} = {}) {
  const tools = Object.freeze(state.tools.map((tool) => describeDirectAction(tool, ownerMode)));
  return Object.freeze({
    connection: 'ready',
    toolTransport: state.toolTransport,
    mode: ownerMode
      ? (state.multimodal ? 'Abliterated Owner + multimodal' : 'Abliterated Owner')
      : (state.multimodal ? 'Universal + multimodal' : 'Universal'),
    ownerMode: Object.freeze({
      enabled: ownerMode,
      profile: ownerMode ? 'owner-autonomous' : 'human-gated',
      authority: ownerMode ? 'extension-local' : 'sidepanel-approval',
    }),
    tab: Object.freeze({
      id: tab.id,
    ...
    snapshot: Object.freeze({
      ...
      navigationGeneration: state.revision,
    }),
    fileInputs: Object.freeze(clone(fileInputs)),
    tools,
    actions: Object.freeze(tools.filter((tool) => tool.classification !== 'read')),
...
export async function createExtensionUniversalRuntime({
  ...
  if (ownerAuthorizationPolicy !== null && typeof ownerAuthorizationPolicy?.decide !== 'function') {
    throw new TypeError('ownerAuthorizationPolicy must expose decide() or be null.');
  }
  ...
  const fileInputsBySession = new Map();
  ...
  function clearOtherSessionCaptureCache(tabId, frameId, sessionId) {
    ...
    for (const [key, entry] of fileInputsBySession.entries()) {
      if (entry.tabId === tabId && entry.frameId === frameId && entry.sessionId !== sessionId) fileInputsBySession.delete(key);
    }
  }
  ...
  async function executePageAction(request) {
    ...
    if (pageRequest.mode === 'read' && pageRequest.preparedAction.effect?.operation === 'scroll') {
      if (response.receipt?.operation !== 'scroll') throw runtimeError('SCROLL_RECEIPT_INVALID', 'The browser did not return a scroll observation.');
      const rawAfter = await requestRawSnapshot(pageRequest.tabId, pageRequest.frameId, pageRequest.sessionId);
      const snapshot = createPageSnapshot(boundedSnapshot(rawAfter).snapshot);
      if (snapshot.metadata.url !== current.metadata.url) throw runtimeError('SCROLL_CONTEXT_CHANGED', 'The page changed during scrolling.');
      await ingestRaw({ tabId: pageRequest.tabId, frameId: pageRequest.frameId, sessionId: pageRequest.sessionId, rawSnapshot: rawAfter });
      return { ...response.receipt, pageFingerprint: snapshot.pageFingerprint, mainText: snapshot.mainText.slice(0, 8000),
        truncated: snapshot.mainText.length > 8000, untrustedContent: true };
    }
  ...
  async function verifyPostcondition({
    ...
    if (descriptor.name === 'publish_x_post' && descriptor.adapter?.id === 'x-post'
      && normalizedVerdict.status === 'unverified'
      && normalizedVerdict.reasonCode === 'X_PUBLICATION_NOT_CONFIRMED' && observationAttempt < 7) {
      // X can render the new post after acknowledging the click. Only repeat
      // the observation within the existing timeout; never repeat the action.
      await new Promise(resolve => setTimeout(resolve, 250));
      return verifyPostcondition({ tabId, frameId, sessionId, signal, descriptor,
        preparedAction, dispatchReceipt, beforeSnapshot }, observationAttempt + 1);
    }
    return {
  ...
  const runtime = createUniversalSessionRuntime({
    ...
    now,
    registerTools: (request) => bridge.registerGeneratedTools({
      ...request,
      tools: request.tools.map((tool) => describeDirectAction(tool, ownerAuthorizationPolicy !== null)),
    }),
    executePageAction,
    postconditionVerifier: verifyPostcondition,
    postconditionTimeoutMs,
    ...(ownerAuthorizationPolicy ? { ownerAuthorizationPolicy } : {}),
  });
  ...
  async function executeContextTool(request) {
    const tool = runtime.state(request.tabId, request.frameId).tools.find((entry) => entry.name === request.name);
    if (!ownerAuthorizationPolicy || !isXDirectAction(tool)) return runtime.executeTool(request);
    const snapshot = await refreshSnapshot(request.tabId, request.frameId, request.sessionId);
    return runtime.executeOwnerAction({ ...request, snapshot });
  }
  ...
  async function ingestRaw({ tabId, frameId, sessionId, rawSnapshot, captureEvidence = null }) {
    ...
    const extractorPageFingerprint = typeof rawSnapshot.pageFingerprint === 'string' && /^[a-f0-9]{64}$/.test(rawSnapshot.pageFingerprint)
      ? rawSnapshot.pageFingerprint
      : null;
    const fileInputs = Array.isArray(rawSnapshot.fileInputs) ? rawSnapshot.fileInputs
      .slice(0, 24)
      .filter((entry) => plainObject(entry) && typeof entry.ref === 'string')
      .map((entry) => Object.freeze({
        ref: entry.ref.slice(0, 256),
        name: typeof entry.name === 'string' ? entry.name.slice(0, 512) : '',
        accept: typeof entry.accept === 'string' ? entry.accept.slice(0, 512) : '',
        multiple: entry.multiple === true,
      })) : [];
    fileInputsBySession.set(captureKey(tabId, frameId, sessionId), Object.freeze({
      tabId, frameId, sessionId, pageFingerprint, extractorPageFingerprint, fileInputs: Object.freeze(fileInputs),
    }));
  ...
  async function activeContext(target = {}) {
    ...
    const frameId = Number.isInteger(target?.targetFrameId) && target.targetFrameId >= 0
      ? target.targetFrameId
      : 0;
    const session = registry.get(tab.id, frameId);
    if (!session) throw runtimeError('SESSION_NOT_FOUND', 'Activate ToolBraid on this tab first.');
    const state = runtime.state(tab.id, frameId);
    const origin = safeOrigin(frameId === 0 ? tab.url : session.url);
    if (session.tabId !== tab.id || session.frameId !== frameId || state.sessionId !== session.sessionId || state.origin !== origin) {
  ...
  async function handleUiMessage(type, payload = {}) {
    ...
    let context = await activeContext(payload);
    if (type === UI_MESSAGE_TYPES.UI_GET_STATE) {
      if (payload.refresh === true) {
        const { tab, session } = context;
        await serializeSnapshotIngest(tab.id, session.frameId, async () => {
          const rawSnapshot = await requestRawSnapshot(tab.id, session.frameId, session.sessionId);
          const pageFingerprint = canonicalPageFingerprint(boundedSnapshot(rawSnapshot).snapshot);
          const extractorPageFingerprint = typeof rawSnapshot.pageFingerprint === 'string' && /^[a-f0-9]{64}$/.test(rawSnapshot.pageFingerprint)
            ? rawSnapshot.pageFingerprint : null;
          const cached = fileInputsBySession.get(captureKey(tab.id, session.frameId, session.sessionId));
          if (runtime.state(tab.id, session.frameId).pageFingerprint !== pageFingerprint
              || cached?.extractorPageFingerprint !== extractorPageFingerprint) {
            await ingestRaw({ tabId: tab.id, frameId: session.frameId, sessionId: session.sessionId, rawSnapshot });
          }
        });
        context = await activeContext(payload);
      }
      const frameId = context.session.frameId;
      const auditTrail = await auditForSession({ tabId: context.tab.id, frameId, sessionId: context.session.sessionId });
      ...
      const capture = clone(captureCache.get(captureKey(context.tab.id, frameId, context.session.sessionId)) ?? null);
      const multimodalProvider = await multimodalSettings.publicState();
      return {
        ok: true,
        state: uiState(context.tab, visibleState(context.tab.id, frameId, context.state), {
          audit,
          capture,
          multimodalProvider,
          ownerMode: ownerAuthorizationPolicy !== null,
          fileInputs: (() => {
            const entry = fileInputsBySession.get(captureKey(context.tab.id, frameId, context.session.sessionId));
            return entry?.pageFingerprint === context.state.pageFingerprint ? entry.fileInputs : [];
          })(),
          extractorPageFingerprint: (() => {
            const entry = fileInputsBySession.get(captureKey(context.tab.id, frameId, context.session.sessionId));
            return entry?.pageFingerprint === context.state.pageFingerprint ? entry.extractorPageFingerprint : null;
          })(),
        }),
      };
    }
    ...
    if (type === UI_MESSAGE_TYPES.UI_REANALYZE_MULTIMODAL) {
      const frameId = context.session.frameId;
      const state = await serializeSnapshotIngest(context.tab.id, frameId, async () => {
        const rawSnapshot = await requestRawSnapshot(context.tab.id, frameId, context.session.sessionId);
        const bounded = boundedSnapshot(rawSnapshot);
    ...
    if (type === UI_MESSAGE_TYPES.UI_EXECUTE_READ) {
      ...
      const viewportRead = tool.effect?.operation === 'scroll';
      const currentTool = viewportRead ? tool : activeReadTool(current, tool.name);
      if (stableStringify(viewportRead ? authorityBinding(current) : readBinding(current)) !== stableStringify(viewportRead ? authorityBinding(context) : binding)
    ...
    if (type === UI_MESSAGE_TYPES.UI_EXECUTE_OWNER_ACTION) {
      if (payload.sessionId !== context.session.sessionId) {
        throw runtimeError('SESSION_DRIFT', 'The owner action belongs to a different active page session.');
      }
      if (typeof payload.toolName !== 'string'
          || !context.state.tools.some((tool) => tool.name === payload.toolName && tool.classification !== 'read')) {
        throw runtimeError('TOOL_NOT_FOUND', 'The selected effectful page action is no longer active.');
      }
      if (!plainObject(payload.arguments ?? {})) {
        throw runtimeError('UI_ACTION_ARGUMENTS_INVALID', 'Owner action arguments must be a plain object.');
      }
      const snapshot = await refreshSnapshot(context.tab.id, context.session.frameId, context.session.sessionId);
    ...
    const pendingActions = runtime.state(context.tab.id, context.session.frameId).pendingActions;
  ...
  async function closeSession(session, reason) {
    ...
    captureCache.delete(key);
    multimodalStateOverrides.delete(key);
    fileInputsBySession.delete(key);
