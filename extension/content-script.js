...
(function installToolBraidContent(global) {
  ...
  function onRuntimeMessage(message, _sender, sendResponse) {
    ...
    if (message.type === protocol.TYPES.PAGE_ACTION_EXECUTE) {
      ...
      const sourceType = prepared?.target?.type ?? prepared?.descriptor?.sourceType;
      const actionArguments = prepared?.normalizedArguments ?? prepared?.arguments ?? {};
      if (classification === 'read' && prepared?.effect?.operation === 'scroll') {
        if (typeof executor.safeScroll !== 'function') {
          sendResponse?.({ ok: false, error: { code: 'SCROLL_UNAVAILABLE', message: 'Reload the updated ToolBraid extension to enable scrolling.' } });
          return false;
        }
        executor.safeScroll({ classification, arguments: actionArguments, preparedAction: prepared,
          binding: { ...targetBinding, ref: prepared.target?.ref ?? null, canonicalPageFingerprint: prepared.pageFingerprint,
            extractorFingerprint: message.extractorPageFingerprint },
        }).then((result) => {
          sendResponse?.(result?.ok ? { ok: true, receipt: result, provenance: protocol.PROVENANCE } : result);
          if (result?.ok) scheduleSnapshot('viewport-scroll');
        }, () => sendResponse?.({ ok: false, error: { code: 'SCROLL_FAILED', message: 'The viewport could not be observed.' } }));
        return true;
      }
      ...
      const operation = prepared?.effect?.operation === 'select-text'
        ? 'select-text'
      ...
      if (operation === 'set' && targetBinding.type === 'contenteditable' && typeof executor.safeEdit === 'function') {
        executor.safeEdit(actionRequest).then((result) => {
          sendResponse?.(result?.ok ? { ok: true, receipt: { ...result, actionId: prepared?.actionId ?? null, mode: message.mode ?? classification, provenance: protocol.PROVENANCE }, provenance: protocol.PROVENANCE } : result);
          scheduleSnapshot('editor-action');
        }).catch((error) => sendResponse?.({ ok: false, error: { code: 'EDITOR_WRITE_FAILED', message: error.message } }));
        return true;
      }
      const result = executor.safeExecute(actionRequest);
      if (!result?.ok) {
    ...
    const timeout = global.setTimeout(() => {
      if (state.pendingResponses.get(requestId) !== respond) return;
      state.pendingResponses.delete(requestId);
      sendResponse({ ok: false, error: { code: 'MAIN_REGISTRATION_TIMEOUT', message: 'The page did not acknowledge WebMCP registration. Reload this page after updating the extension.' } });
    }, 3000);
    const respond = (response) => {
      global.clearTimeout(timeout);
      sendResponse(response);
    };
    state.pendingResponses.set(requestId, respond);
    if (!postToMain(parsed.value)) {
      state.pendingResponses.delete(requestId);
      respond({ ok: false, error: { code: 'MAIN_INJECTOR_UNAVAILABLE', message: 'ToolBraid MAIN injector is unavailable.' } });
    }
    return true;
  }
