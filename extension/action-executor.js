...
(function installToolBraidActionExecutor(global) {
  ...
  const FILE_TARGET_MARKER_ATTRIBUTE = 'data-toolbraid-file-target';
  ...
  function semanticRoots(documentRef) {
    ...
    let xMain = null;
    let xNavigation = null;
    let editors = [];
    let publishControls = [];
    let dialogs = [];
    try {
      const url = new URL(documentRef.location?.href || documentRef.URL);
      if (url.protocol === 'https:' && ['x.com', 'www.x.com', 'twitter.com', 'www.twitter.com'].includes(url.hostname)) {
        xMain = safeCall(documentRef, 'querySelector', ['main'], null);
        xNavigation = safeCall(documentRef, 'querySelector', ['nav'], null);
        editors = toArray(safeCall(documentRef, 'querySelectorAll', [
          'main [contenteditable="true"], [role="dialog"] [contenteditable="true"], main textarea, [role="dialog"] textarea, main input[type="text"], [role="dialog"] input[type="text"]',
        ], [])).filter((element) => isVisible(element, documentRef)).slice(0, 8);
        publishControls = toArray(safeCall(documentRef, 'querySelectorAll', ['[data-testid="tweetButton"], [data-testid="tweetButtonInline"]'], []))
          .filter((element) => isVisible(element, documentRef)).slice(0, 8);
        dialogs = toArray(safeCall(documentRef, 'querySelectorAll', ['[role="dialog"]'], []))
          .filter((element) => isVisible(element, documentRef)).slice(0, 2);
      }
    } catch { /* Keep the generic traversal for unavailable locations. */ }
    const roots = [];
    for (const root of [...publishControls, ...editors, ...dialogs, preferredArticle, main || xMain, xNavigation, documentElement || body]) {
  ...
  function implicitRole(element) {
    ...
    return isContentEditable(element) ? 'textbox' : '';
  }

  function isContentEditable(element) {
    const explicit = attr(element, 'contenteditable', null);
    if (explicit !== null) {
      const value = String(explicit).trim().toLowerCase();
      if (value === 'false') return false;
      if (value === '' || value === 'true' || value === 'plaintext-only') return true;
    }
    return safeGet(element, 'isContentEditable', false) === true;
  }
  ...
  function isVisible(element, documentRef) {
    ...
    while (current && !seen.has(current)) {
      seen.add(current);
      if (hasAttr(current, 'hidden') || attr(current, 'aria-hidden', '').toLowerCase() === 'true') return false;
      const inline = String(safeGet(current, 'style', '')?.cssText ?? attr(current, 'style', '')).toLowerCase();
      if (/(?:^|;)\s*display\s*:\s*none(?:\s*!important)?\s*(?:;|$)/.test(inline)
        || /(?:^|;)\s*visibility\s*:\s*(?:hidden|collapse)(?:\s*!important)?\s*(?:;|$)/.test(inline)) return false;
      const view = safeGet(documentRef, 'defaultView', null);
      const computed = safeCall(view, 'getComputedStyle', [current], null);
      if (computed && (safeGet(computed, 'display', '') === 'none'
        || ['hidden', 'collapse'].includes(safeGet(computed, 'visibility', '')))) return false;
      current = semanticParent(current);
    }
    return true;
  }
  ...
  function eventObject(documentRef, type, options = {}) {
    const view = safeGet(documentRef, 'defaultView', null);
    const EventCtor = options.input === true ? (view?.InputEvent || global.InputEvent || view?.Event || global.Event) : (view?.Event || global.Event);
    try {
      if (typeof EventCtor === 'function') return new EventCtor(type, {
        bubbles: true,
  ...
  function dispatch(element, documentRef, type, events, options) {
    if (typeof element?.dispatchEvent !== 'function') throw executionError('DOM_EVENT_UNAVAILABLE', `Target cannot dispatch ${type}.`);
    const event = eventObject(documentRef, type, options);
    const accepted = element.dispatchEvent(event);
    events.push(type);
    return accepted !== false;
  }

  function setNativeProperty(element, property, value, readonlyMessage) {
    let prototype = Object.getPrototypeOf(element);
    while (prototype) {
      const descriptor = Object.getOwnPropertyDescriptor(prototype, property);
      if (typeof descriptor?.set === 'function') {
        try { descriptor.set.call(element, value); return; } catch { throw executionError('ACTION_TARGET_READONLY', readonlyMessage); }
      }
      prototype = Object.getPrototypeOf(prototype);
    }
    try { element[property] = value; } catch { throw executionError('ACTION_TARGET_READONLY', readonlyMessage); }
  }
  ...
  function normalizedSetValue(element, value) {
    ...
    if (type === 'select') {
      const optionValues = toArray(safeGet(element, 'options', [])).map((option) => String(safeGet(option, 'value', '')));
      if (Array.isArray(value)) {
        ...
        if (values.some((entry) => !optionValues.includes(entry))) throw executionError('ACTION_OPTION_NOT_FOUND', 'Every selected value must match an exact live option.');
        return { property: 'value', value: values, kind: 'select-multiple' };
      } else if (typeof value !== 'string' && typeof value !== 'number') {
        throw executionError('ACTION_INPUT_INVALID', 'Select staging requires a string or number value.');
      } else {
        const selected = String(value);
        if (!optionValues.includes(selected)) throw executionError('ACTION_OPTION_NOT_FOUND', 'The selected value must match an exact live option.');
        return { property: 'value', value: selected, kind: 'value' };
      }
    }
    if (type === 'contenteditable') {
      if (typeof value !== 'string' && typeof value !== 'number') throw executionError('ACTION_INPUT_INVALID', 'Editable text requires a string or number value.');
      return { property: 'textContent', value: String(value), kind: 'text' };
    }
  ...
  function setValue(element, value) {
    ...
    if (normalized.kind === 'checked') {
      setNativeProperty(element, 'checked', normalized.value, 'Checkbox target is not writable.');
    ...
    if (normalized.kind === 'select-multiple') {
      ...
      for (const option of toArray(safeGet(element, 'options', []))) {
        setNativeProperty(option, 'selected', wanted.has(String(safeGet(option, 'value', ''))), 'Select target is not writable.');
      }
      return { property: normalized.property, value: normalized.value.slice() };
    }
    if (normalized.kind === 'text') {
      try { element.textContent = normalized.value; } catch { throw executionError('ACTION_TARGET_READONLY', 'Editable text target is not writable.'); }
      return { property: normalized.property, value: normalized.value };
    }
    setNativeProperty(element, 'value', normalized.value, 'Input target is not writable.');
    return { property: normalized.property, value: normalized.value };
  }

  function selectEditorContents(element, documentRef) {
    const selection = documentRef.defaultView?.getSelection?.();
    if (!selection || typeof element.focus !== 'function') throw executionError('SELECTION_UNAVAILABLE', 'This editor cannot expose a browser editing range.');
    element.focus();
    const range = documentRef.createRange();
    const walker = documentRef.createTreeWalker(element, 4);
    const first = walker.nextNode();
    let last = first;
    let next;
    let visited = 0;
    while ((next = walker.nextNode())) {
      if (++visited > 4096) throw executionError('SELECTION_LIMIT', 'The editor exceeds the bounded selection limit.');
      last = next;
    }
    if (first) {
    ...
    synchronizeEditorSelection(element, documentRef);
  }

  function synchronizeEditorSelection(element, documentRef) {
    const view = documentRef.defaultView;
    documentRef.dispatchEvent(new view.Event('selectionchange'));
    // Draft.js also uses mouseup to reconcile browser selections on Chrome.
    if (element.querySelector?.('[data-offset-key]')) {
      element.dispatchEvent(new view.MouseEvent('mouseup', { bubbles: true, view }));
    }
  }

  function applyValueWithEvents(element, value, documentRef, events) {
    if (fieldType(element) === 'contenteditable') {
      const normalized = normalizedSetValue(element, value);
      if (typeof documentRef.execCommand === 'function') {
        selectEditorContents(element, documentRef);
        const view = documentRef.defaultView;
        const xEditor = /^(?:composer|tweetTextarea_\d+)$/.test(attr(element, 'data-testid', ''))
          && /^https:\/\/(?:www\.)?(?:x|twitter)\.com\//i.test(String(documentRef.location?.href ?? documentRef.URL ?? ''));
        if (xEditor && normalized.value && typeof view?.DataTransfer === 'function' && typeof view?.ClipboardEvent === 'function') {
          const clipboardData = new view.DataTransfer();
          clipboardData.setData('text/plain', normalized.value);
          const paste = new view.ClipboardEvent('paste', { bubbles: true, cancelable: true, clipboardData });
          const handled = !element.dispatchEvent(paste);
          events.push('paste');
          if (handled) return { property: normalized.property, value: normalized.value };
        }
        // Native editing keeps rich-editor state and undo history in sync.
        if (!documentRef.execCommand(normalized.value ? 'insertText' : 'delete', false, normalized.value)) {
          throw executionError('ACTION_INPUT_CANCELLED', 'The browser rejected the editor command; no DOM replacement was attempted.');
        }
        events.push(normalized.value ? 'native:insertText' : 'native:delete');
        return { property: normalized.property, value: normalized.value };
      }
      const accepted = dispatch(element, documentRef, 'beforeinput', events, { input: true, data: normalized.value, inputType: 'insertText' });
      if (!accepted) throw executionError('ACTION_INPUT_CANCELLED', 'Editable text rejected the beforeinput event.');
    }
    const changed = setValue(element, value);
    dispatch(element, documentRef, 'input', events, { input: true, data: String(value), inputType: 'insertText', cancelable: false });
  ...
  function readValue(element) {
    ...
    if (type === 'contenteditable') return { value: safeGet(element, 'textContent', '') };
  ...
  function operationFor(request, classification, element) {
    ...
    if (['input', 'select', 'textarea'].includes(tagName(element)) || isContentEditable(element)) return 'set';
    return 'click';
  }
  ...
  function isFormField(element) {
    ...
    return INTERACTIVE_ROLES.has(role) || isContentEditable(element);
  }
  ...
  function captureValue(element) {
    ...
    const capture = {
      ...
      textContent: fieldType(element) === 'contenteditable' ? safeGet(element, 'textContent', '') : undefined,
    };
    if (type === 'select') {
  ...
  function restoreValue(element, capture) {
    try {
      ...
      } else if (capture.type === 'contenteditable') {
        element.textContent = capture.textContent;
  ...
  function applyFormArguments(request, form, elements, refs, byId, documentRef, events) {
    ...
    try {
      for (const entry of plan) {
        applyValueWithEvents(entry.element, entry.value, documentRef, events);
        changed.push({
          key: entry.key,
  ...
  function executeAction(request = {}, prepareEditing = false) {
    ...
    if (classification !== CLASS_READ && !isVisible(element, documentRef)) throw executionError('ACTION_TARGET_NOT_VISIBLE', 'Page actions require an exact visible target.');
    if (classification !== CLASS_READ && (safeGet(element, 'disabled', false) === true || hasAttr(element, 'disabled') || attr(element, 'aria-disabled', '').toLowerCase() === 'true')) {
      throw executionError('ACTION_TARGET_DISABLED', 'Page actions cannot execute against a disabled target.');
    }
    if (classification === CLASS_MUTATE) requireBinding(request, binding, live);

    if (prepareEditing && classification !== CLASS_READ && operation === 'set' && isContentEditable(element) && typeof documentRef.execCommand === 'function') {
      const expectedText = normalizedSetValue(element, argumentValue(request, liveTarget, element)).value;
      const directXPaste = /^tweetTextarea_\d+$/.test(attr(element, 'data-testid', ''))
        && /^https:\/\/(?:www\.)?(?:x|twitter)\.com\//i.test(String(documentRef.location?.href ?? documentRef.URL ?? ''))
        && typeof documentRef.defaultView?.DataTransfer === 'function'
        && typeof documentRef.defaultView?.ClipboardEvent === 'function';
      // X expands a fresh reply editor on focus. Its paste handler can focus
      // and edit in the final validated pass, without invalidating our own binding.
      if (!directXPaste) selectEditorContents(element, documentRef);
      return { ok: true, editPreparation: { element, documentRef, expectedText, url: String(documentRef.location?.href ?? documentRef.URL) } };
    }
    ...
    if (classification === CLASS_STAGE) {
      ...
      if (!['input', 'textarea', 'select'].includes(tagName(element)) && !isContentEditable(element) && !['checkbox', 'radio', 'switch', 'textbox'].includes(liveTarget.role)) {
        throw executionError('ACTION_TARGET_UNSUPPORTED', 'Stage actions require an input, select, textarea, editable text, checkbox, radio, or switch target.');
      }
      ...
      const events = [];
      const changed = applyValueWithEvents(element, value, documentRef, events);
      return {
        ok: true,
        version: VERSION,
        classification,
    ...
    if (!['click', 'submit', 'set', 'select-text'].includes(operation)) throw executionError('ACTION_OPERATION_INVALID', 'Mutation actions support only click, submit, an approved value set, or exact text selection.');
    const events = [];
    let changed = { operation };
    if (operation === 'select-text') {
      const input = actionArguments(request);
      if (!isContentEditable(element) || typeof input.text !== 'string' || !input.text || input.text.length > 4000
        || Object.keys(input).some((key) => !['text', 'occurrence'].includes(key))
        || !Number.isInteger(input.occurrence ?? 1) || (input.occurrence ?? 1) < 1 || (input.occurrence ?? 1) > 100) {
        throw executionError('SELECTION_ARGUMENTS_INVALID', 'Select bounded exact text in the bound editable article body.');
      }
      const walker = documentRef.createTreeWalker?.(element, 4);
      const nodes = [];
      let content = '';
      ...
      while (walker && (node = walker.nextNode())) {
        if (nodes.length >= 4096 || content.length + node.data.length > 100000) throw executionError('SELECTION_LIMIT', 'The editor exceeds the bounded text selection limit.');
        nodes.push({ node, start: content.length });
        content += node.data;
      }
      let start = -1;
      for (let count = 0; count < (input.occurrence ?? 1); count += 1) {
        start = content.indexOf(input.text, start < 0 ? 0 : start + input.text.length);
        if (start < 0) throw executionError('SELECTION_TEXT_MISSING', 'The exact text occurrence is no longer in the editor.');
      }
      const end = start + input.text.length;
      const first = nodes.find((entry) => start >= entry.start && start < entry.start + entry.node.data.length);
      const last = nodes.find((entry) => end > entry.start && end <= entry.start + entry.node.data.length);
      const selection = documentRef.defaultView?.getSelection?.();
      if (!first || !last || !selection) throw executionError('SELECTION_UNAVAILABLE', 'This editor cannot expose an exact browser text range.');
      element.focus();
      const range = documentRef.createRange();
      ...
      selection.addRange(range);
      synchronizeEditorSelection(element, documentRef);
      if (selection.getRangeAt(0).cloneContents().textContent !== input.text) throw executionError('SELECTION_NOT_CONFIRMED', 'The selected text did not match the request.');
      changed = { operation: 'select-text', applied: true };
    } else if (operation === 'set') {
      if (!['input', 'textarea', 'select'].includes(tagName(element)) && !isContentEditable(element) && !['checkbox', 'combobox', 'radio', 'slider', 'switch', 'textbox'].includes(liveTarget.role)) {
        throw executionError('ACTION_TARGET_UNSUPPORTED', 'Approved value changes require an input, select, textarea, editable text, checkbox, radio, or switch target.');
      }
      const value = argumentValue(request, liveTarget, element);
      ...
      const applied = applyValueWithEvents(element, value, documentRef, events);
  ...
  function fileTargetRequest(request) {
    if (!isPlain(request)) throw executionError('FILE_TARGET_REQUEST_INVALID', 'File target request must be a plain object.');
    const documentRef = documentFrom(request);
    const marker = typeof request.marker === 'string' ? request.marker.trim() : '';
    if (!marker) throw executionError('FILE_TARGET_MARKER_REQUIRED', 'A non-empty temporary file target marker is required.');
    const maxNodes = Number.isInteger(request.maxNodes) && request.maxNodes > 0 ? request.maxNodes : 1024;
    const resolved = targetElements(documentRef, request.ref, maxNodes);
    const liveSnapshot = snapshotFromExtractor(documentRef, request);
    const liveFingerprint = liveSnapshot.pageFingerprint || fingerprintPageSnapshot(liveSnapshot);
    if (typeof request.pageFingerprint !== 'string' || !request.pageFingerprint) {
      throw executionError('FILE_TARGET_FINGERPRINT_REQUIRED', 'An exact page fingerprint is required for file targets.');
    }
    if (request.pageFingerprint !== liveFingerprint) {
      throw executionError('PAGE_FINGERPRINT_DRIFT', 'The live extractor fingerprint no longer matches the file target binding.', { expected: request.pageFingerprint, actual: liveFingerprint });
    }
    return { documentRef, marker, liveFingerprint, ...resolved };
  }

  function assertLiveFileInput(element, documentRef) {
    if (tagName(element) !== 'input' || fieldType(element) !== 'file') {
      throw executionError('FILE_TARGET_TYPE_INVALID', 'File targets require an input[type=file].');
    }
    if (!isVisible(element, documentRef)) throw executionError('ACTION_TARGET_NOT_VISIBLE', 'File targets must be visible.');
    if (safeGet(element, 'disabled', false) === true || hasAttr(element, 'disabled') || attr(element, 'aria-disabled', '').toLowerCase() === 'true') {
      throw executionError('ACTION_TARGET_DISABLED', 'File targets cannot be disabled.');
    }
  }

  function normalizeAccept(value) {
    return String(value || '').split(',').map((entry) => entry.trim().toLowerCase()).filter(Boolean).join(',');
  }

  function validateFileDescriptor(element, request) {
    const accept = normalizeAccept(attr(element, 'accept', ''));
    const multiple = safeGet(element, 'multiple', false) === true || hasAttr(element, 'multiple');
    if (request.expectedAccept !== undefined && normalizeAccept(request.expectedAccept) !== accept) {
      throw executionError('FILE_TARGET_ACCEPT_MISMATCH', 'The file target accept descriptor changed.', { expected: normalizeAccept(request.expectedAccept), actual: accept });
    }
    if (request.expectedMultiple !== undefined && Boolean(request.expectedMultiple) !== multiple) {
      throw executionError('FILE_TARGET_MULTIPLE_MISMATCH', 'The file target multiple descriptor changed.', { expected: Boolean(request.expectedMultiple), actual: multiple });
    }
  ...
  function revalidateFileTarget(request = {}) {
    const state = fileTargetRequest(request);
    assertLiveFileInput(state.element, state.documentRef);
    validateFileDescriptor(state.element, request);
  ...
  function cleanupFileTarget(request = {}) {
    try {
      if (!isPlain(request)) return { ok: false, removed: false };
      const documentRef = documentFrom(request);
      const marker = typeof request.marker === 'string' ? request.marker.trim() : '';
      ...
      return { ok: true, removed: attr(resolved.element, FILE_TARGET_MARKER_ATTRIBUTE, null) === null };
    } catch {
      return { ok: false, removed: false };
    }
  }

  async function safeScroll(request) {
    try {
      if (!isPlain(request) || request.classification !== CLASS_READ
        || request.preparedAction?.effect?.operation !== 'scroll'
        || request.preparedAction?.provenance?.source !== 'toolbraid.universal') {
        throw executionError('SCROLL_REQUEST_INVALID', 'Scrolling requires an exact generated viewport tool.');
      }
      const input = request.arguments ?? {};
      const directions = ['up', 'down', 'left', 'right', 'top', 'bottom'];
      if (!isPlain(input) || !directions.includes(input.direction)
        || Object.keys(input).some((key) => !['direction', 'pixels'].includes(key))
        || (input.pixels !== undefined && (!Number.isInteger(input.pixels) || input.pixels < 1 || input.pixels > 4000))
        || (['top', 'bottom'].includes(input.direction) && input.pixels !== undefined)) {
        throw executionError('SCROLL_ARGUMENTS_INVALID', 'Choose a scroll direction and optionally 1–4000 pixels; omit pixels for top/bottom.');
      }
      const documentRef = documentFrom(request);
      const binding = expectedBinding(request);
      assertRequestBindingConsistent(request, binding);
      const snapshot = snapshotFromExtractor(documentRef, request);
      const pageViewport = !binding.ref && request.preparedAction.target?.type === 'page';
      if (pageViewport && request.preparedAction.provenance.url !== snapshot.metadata?.url) {
        throw executionError('SCROLL_CONTEXT_CHANGED', 'The page changed before scrolling.');
      }
      if (!binding.extractorFingerprint || (!pageViewport && binding.extractorFingerprint !== snapshot.pageFingerprint)) {
        throw executionError('PAGE_FINGERPRINT_DRIFT', 'The page changed before scrolling. Refresh its tools.');
      }
      const root = documentRef.scrollingElement || documentRef.documentElement;
      const resolved = binding.ref ? targetElements(documentRef, binding.ref, 1024) : null;
      const element = resolved?.element || root;
      if (!element || !isVisible(element, documentRef)) throw executionError('SCROLL_TARGET_UNAVAILABLE', 'The scroll target is no longer visible.');
      if (resolved) {
        const target = snapshot.elementRefs.find((entry) => entry.ref === binding.ref);
        if (!target || (target.role ?? null) !== (binding.role ?? null) || target.name !== binding.name) throw executionError('ACTION_TARGET_DRIFT', 'The scroll target changed.');
      }
      const style = safeCall(documentRef.defaultView, 'getComputedStyle', [element], null);
      const horizontal = ['left', 'right'].includes(input.direction);
      if (resolved && !/^(auto|scroll)$/.test(horizontal ? style?.overflowX : style?.overflowY)) {
        throw executionError('SCROLL_TARGET_UNAVAILABLE', 'The selected container does not scroll on that axis.');
      }
      const position = () => {
        ...
        return { top: element.scrollTop, left: element.scrollLeft,
          ...
          minLeft: style?.direction === 'rtl' ? -overflowX : 0,
          maxLeft: style?.direction === 'rtl' ? 0 : overflowX };
      };
      const before = position();
      if (Object.values(before).some((value) => !Number.isFinite(value))) throw executionError('SCROLL_METRICS_UNAVAILABLE', 'Browser scroll metrics are unavailable.');
      const distance = input.pixels ?? Math.min(4000, Math.max(1, Math.floor((horizontal ? before.width : before.height) * 0.8)));
      let top = before.top;
      let left = before.left;
      if (input.direction === 'top') top = 0;
      else if (input.direction === 'bottom') top = before.maxTop;
      else if (horizontal) left += input.direction === 'left' ? -distance : distance;
      else top += input.direction === 'up' ? -distance : distance;
      top = Math.max(0, Math.min(before.maxTop, top));
      left = Math.max(before.minLeft, Math.min(before.maxLeft, left));
      if (typeof element.scrollTo !== 'function') throw executionError('SCROLL_UNAVAILABLE', 'This browser target has no scroll implementation.');
      element.scrollTo({ top, left, behavior: 'instant' });
      await new Promise((resolve) => global.setTimeout(resolve, 250));
      if (element.isConnected === false || String(documentRef.location?.href ?? documentRef.URL) !== snapshot.metadata.url) {
        throw executionError('SCROLL_CONTEXT_CHANGED', 'The document or target changed while scrolling; inspect the current page before continuing.');
      }
      const after = position();
      const moved = Math.abs(after.top - before.top) > 0.5 || Math.abs(after.left - before.left) > 0.5;
      const atBoundary = input.direction === 'up' || input.direction === 'top' ? after.top <= 1
        : input.direction === 'down' || input.direction === 'bottom' ? after.top >= after.maxTop - 1
          : input.direction === 'left' ? after.left <= after.minLeft + 1 : after.left >= after.maxLeft - 1;
      return { ok: true, operation: 'scroll', status: moved ? 'scrolled' : atBoundary ? 'at-boundary' : 'not-moved',
        ref: binding.ref ?? null, direction: input.direction, before, after, moved, atBoundary,
        requestedPosition: { top, left }, scope: 'viewport-only', events: [] };
    } catch (error) {
      return { ok: false, error: { code: error?.code || 'SCROLL_FAILED', message: error?.message || 'Scrolling failed.' } };
    }
  }

  async function safeEdit(request) {
    try {
      const prepared = executeAction(request, true);
      if (!prepared.editPreparation) return prepared;
      const { element, documentRef, expectedText, url } = prepared.editPreparation;
      // Rich editors reconcile their selection on the next browser event turn.
      await new Promise(resolve => global.setTimeout(resolve, 80));
      if (element.isConnected === false || String(documentRef.location?.href ?? documentRef.URL) !== url) throw executionError('ACTION_TARGET_DRIFT', 'The editor changed before text entry.');
      const result = safeExecute(request);
      if (!result.ok) return result;
      await new Promise(resolve => global.setTimeout(resolve, 80));
      const actual = String(element.innerText ?? element.textContent ?? '').replace(/\s+/g, ' ').trim();
      if (element.isConnected === false || actual !== expectedText.replace(/\s+/g, ' ').trim()) throw executionError('EDITOR_TEXT_MISMATCH', 'The editor did not retain the exact requested text. Inspect the draft before another write.');
      return result;
    } catch (error) {
      return { ok: false, error: { code: error?.code || 'EDITOR_WRITE_FAILED', message: error?.message || 'Editor write failed.' } };
    }
  }
