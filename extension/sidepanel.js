...
function normalizeAction(action, index) {
  ...
  return {
    ...
    effect,
    inputSchema: schema,
    directExecution: action?.directExecution === true,
...
function normalizeState(source = {}, localApprovals = []) {
  ...
  const normalized = {
    ...
    toolTransport: boundedText(source.toolTransport, '', 64),
    ...
    ownerMode: {
      active: connection === 'ready'
        && (source.ownerMode?.active === true || source.ownerMode?.enabled === true),
      controlPlane: boundedText(source.ownerMode?.controlPlane, 'Local', 80),
      modelRoute: boundedText(source.ownerMode?.modelRoute ?? source.ownerMode?.model, 'Local / cloud via MCP', 120),
      policy: boundedText(source.ownerMode?.policy ?? source.ownerMode?.profile, 'owner-autonomous', 80),
    },
  };
...
export function createUiController({
  ...
  async function setMissionPhase(missionId, phase, event) {
    ...
    const mission = latest.missions.find((entry) => entry.missionId === id);
...
function renderAction(action, controller, onChanged) {
  ...
  const direct = action.directExecution === true && controller.getState().ownerMode.active;
  const card = makeElement('article', 'item-card');
  ...
  metadata.append(makeElement('span', 'risk-badge', direct ? 'X direct control · enabled' : (isStage ? 'Local only · no external effect' : (action.requiresApproval ? 'Approval required' : 'Read only'))));
  card.append(metadata);
  ...
  for (const [name, schema] of Object.entries(properties).slice(0, 32)) addInputField(form, name, plainObject(schema) ? schema : {});
  const footer = makeElement('div', 'action-footer');
  const prepare = makeElement('button', 'button button-primary', direct ? 'Run X action' : (isStage ? 'Apply local stage' : 'Prepare exact action'));
  prepare.type = 'submit';
  ...
  form.addEventListener('submit', async (event) => {
    event.preventDefault();
    if (!trustedEvent(event)) return;
    const response = await withButtonBusy(prepare, direct ? 'Executing…' : (isStage ? 'Applying…' : 'Preparing…'), () => controller.prepareAction(action.id, readArguments(form, action.inputSchema)));
     onChanged(response, response.ok === true
       ? (response.result?.status === 'approval-required' ? 'Owner policy requires approval for this action.'
         : response.result?.status === 'denied' ? 'X action was not dispatched: owner policy denied it.'
         : direct ? 'X action dispatched. See its receipt for the verified outcome.'
...
function workflowFor(state) {
  ...
  if (actionableApprovals.length) return { now: 'Manual fallback waiting', next: 'Review the policy exception', required: 'Approve or deny fallback' };
  if (state.pendingActions.length || state.missions.some((mission) => mission.pendingActions.length)) return { now: 'Action prepared', next: state.ownerMode.active ? 'Owner policy will dispatch it' : 'Review its exact scope', required: state.ownerMode.active ? 'Nothing unless fallback is requested' : 'Approve or deny' };
...
export function createSidepanelApp({
  ...
  const refs = {
    ...
    ownerModeBadge: documentRef.getElementById('owner-mode-badge'),
    ownerModeSummary: documentRef.getElementById('owner-mode-summary'),
    ownerControlPlane: documentRef.getElementById('owner-control-plane'),
    ownerModelRoute: documentRef.getElementById('owner-model-route'),
  ...
  function render(state) {
    ...
    if (refs.ownerModeBadge) {
      refs.ownerModeBadge.textContent = state.ownerMode.active ? 'Active' : 'Fallback only';
      refs.ownerModeBadge.className = `status-badge ${state.ownerMode.active ? 'status-ready' : 'status-pending'}`;
    }
    if (refs.ownerModeSummary) refs.ownerModeSummary.textContent = state.ownerMode.active
      ? `Direct execution is enabled under the local ${state.ownerMode.policy} policy.`
      : 'Owner Autonomy is unavailable; manual approval fallback remains active.';
    if (refs.ownerControlPlane) refs.ownerControlPlane.textContent = state.ownerMode.controlPlane;
    if (refs.ownerModelRoute) refs.ownerModelRoute.textContent = state.ownerMode.modelRoute;
    if (refs.connection) {
    ...
    if (refs.mode) refs.mode.textContent = state.toolTransport === 'extension-mcp' ? 'Extension / MCP' : boundedText(state.mode, 'Waiting', 64);
    ...
    if (refs.announcer && state.connection === 'ready') {
      const announcement = `Updated: Owner Autonomy ${state.ownerMode.active ? 'active' : 'in fallback mode'}. ${workflow.now}. ${state.missions.length} missions, ${state.handoffs.length} intervention steps, ${state.tools.length} tools, ${state.actions.length} actions, ${state.approvals.length} approval records, ${state.evidence.stats.total} evidence items, ${state.receipts.length} receipts.`;
  ...
  async function refresh() {
    if (PUBLIC_RELEASE && !await refreshPublicAccess()) return null;
  ...
  async function initialRefresh() {
    ...
    for (let attempt = 0; attempt <= INITIAL_REFRESH_DELAYS_MS.length; attempt += 1) {
      if (PUBLIC_RELEASE && !publicAccess.enabled) return null;
  ...
  function renderAccess() {
    if (!PUBLIC_RELEASE || !accessRefs.panel) return;
    accessRefs.panel.hidden = false;
    accessRefs.status.textContent = publicAccess.enabled ? 'Enabled' : 'Paused';
    accessRefs.status.className = `status-badge ${publicAccess.enabled ? 'status-ready' : 'status-pending'}`;
    accessRefs['consent-label'].hidden = publicAccess.enabled;
    accessRefs.pause.hidden = !publicAccess.enabled;
    ...
    accessRefs.debugger.className = `status-badge ${debuggerGranted && publicAccess.enabled ? 'status-ready' : 'status-pending'}`;
    if (!publicAccess.enabled) {
      refs.connection.textContent = 'Paused';
      refs.connection.className = 'status-badge status-pending';
  ...
  async function refreshPublicAccess() {
    try {
      const response = await browser.runtime.sendMessage({ type: 'UI_GET_ACCESS' });
      if (response?.ok !== true) throw new Error(response?.error?.message ?? 'Unable to read AI control state.');
      publicAccess = response.access;
      accessTab = response.tab;
      debuggerGranted = await browser.permissions.contains({ permissions: ['debugger'] }).catch(() => false);
    } catch (error) {
      publicAccess = { enabled: false };
      accessTab = null;
      toast(error.message, true);
    }
    if (!publicAccess.enabled) render(normalizeState({}));
    renderAccess();
  ...
  accessRefs.enable?.addEventListener('click', async (event) => {
    ...
    const target = { targetTabId: accessTab.id, origin: accessTab.origin };
    // Begin the browser grant in the actual button gesture, before other awaits.
    const grant = browser.permissions.request({ origins: [`${target.origin}/*`] });
    await withButtonBusy(accessRefs.enable, 'Connecting…', async () => {
      try {
        if (!await grant) throw new Error('Site access was not granted. AI control was not enabled.');
        if (!publicAccess.enabled) {
          const enabled = await browser.runtime.sendMessage({ type: 'UI_SET_ACCESS', payload: { enabled: true } });
          if (enabled?.ok !== true) throw new Error(enabled?.error?.message ?? 'Unable to enable AI control.');
        }
        await refreshPublicAccess();
        const connected = await browser.runtime.sendMessage({ type: 'UI_CONNECT_SITE', payload: target });
        if (connected?.ok !== true) throw new Error(connected?.error?.message ?? 'Unable to connect this site.');
        await initialRefresh();
      } catch (error) { toast(error.message, true); }
    });
    renderAccess();
  });
  accessRefs.pause?.addEventListener('click', async (event) => {
    if (!PUBLIC_RELEASE || !trustedEvent(event)) return;
    await withButtonBusy(accessRefs.pause, 'Pausing…', async () => {
      try {
        const response = await browser.runtime.sendMessage({ type: 'UI_SET_ACCESS', payload: { enabled: false } });
        if (response?.ok !== true) throw new Error(response?.error?.message ?? 'Unable to save the paused state.');
        accessRefs.consent.checked = false;
      } catch (error) { toast(error.message, true); }
      await refreshPublicAccess();
    });
  });
