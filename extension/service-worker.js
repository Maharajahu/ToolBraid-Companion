import {
  MESSAGE_TYPES,
  PROVENANCE,
  ProtocolError,
  createEnvelope,
  errorPayload,
  isInjectableUrl,
} from './protocol.js';
import { createUniversalBridge } from './bridge.js';
import { TabLifecycleRegistry, sessionBinding } from './lifecycle.js';
import { createChromeStorageAdapter } from '../src/persistence/index.js';
import { createSiteAdapterRegistry } from '../src/site-adapters/registry.js';
import { createXPostAdapter } from '../src/site-adapters/x.js';
import { createYouTubeAdapter } from '../src/site-adapters/youtube.js';
import { createInstagramAdapter } from '../src/site-adapters/instagram.js';
import { createRedditAdapter } from '../src/site-adapters/reddit.js';
import { createWhatsAppWebAdapter } from '../src/site-adapters/whatsapp.js';
import { createTelegramWebAdapter } from '../src/site-adapters/telegram.js';
import { createDiscordWebAdapter } from '../src/site-adapters/discord.js';
import { HANDOFF_STATES, syntheticUiIntent } from '../src/runtime/handoff-broker.js';
import {
  MAX_LEASE_TTL_MS,
  OWNER_PROFILE,
  createLocalOwnerLease,
  createOwnerAutonomyPolicy,
} from '../src/policy/index.js';
import {
  HANDOFF_UI_MESSAGE_TYPES,
  createExtensionHandoffRuntime,
  isHandoffUiMessageType,
} from './handoff-runtime.js';
import {
  createExtensionMissionRuntime,
  isMissionUiMessageType,
} from './mission-runtime.js';
import {
  asExtensionRuntimeError,
  createExtensionUniversalRuntime,
  isUiMessageType,
  UI_MESSAGE_TYPES,
} from './universal-runtime.js';
import { clickVisibleCaptchaCheckbox } from './captcha-checkbox.js';
import { createExtensionMcpEndpoint, installNativeMcpBridge } from './native-mcp-bridge.js';
import { createWebMcpOwnerTools } from './webmcp-owner-tools.js';
import { createCommunityTools, COMMUNITY_ALARM } from './community-tools.js';
import { createBrowserOwnerTools } from './browser-owner-tools.js';
import { createFrameOwnerTools } from './frame-owner-tools.js';
import { createFileUploadController } from './file-upload-controller.js';
import { createDownloadOwnerTools } from './download-owner-tools.js';
import { createHumanVerificationOwnerTools } from './human-verification-owner-tools.js';
import { createVisualOwnerTools } from './visual-owner-tools.js';
import { createResilienceOwnerTools } from './resilience-owner-tools.js';
import { createAccessibilityOwnerTools } from './accessibility-owner-tools.js';
import { createResearchOwnerTools } from './research-owner-tools.js';
import { createMemoryOwnerTools } from './memory-owner-tools.js';
import { createOpenAiCompatibleVisionAdapter } from '../src/multimodal/adapters/openai-compatible.js';
import { PUBLIC_RELEASE } from './product.js';

const PAGE_LIFECYCLE_PORT = 'toolbraid:page-lifecycle';
const HANDOFF_KEY_STORAGE_KEY = 'toolbraid.extension.handoff.key.v1';
const HANDOFF_SURFACE_KIND = 'toolbraid.sidepanel-created-handoff-surface';
const HANDOFF_SURFACE_CREATOR = 'sidepanel';
const MEMORY_STORAGE_KEY = 'toolbraid.personal.memory.v1';
const PUBLIC_ACCESS_KEY = 'toolbraid.public.access.v1';
const SAFE_ERROR_CODE = /^[A-Za-z0-9][A-Za-z0-9._:-]{0,63}$/;
const POSTCONDITION_ADAPTERS = createSiteAdapterRegistry({
  adapters: [
    createXPostAdapter(),
    createYouTubeAdapter(),
    createInstagramAdapter(),
    createRedditAdapter(),
    createWhatsAppWebAdapter(),
    createTelegramWebAdapter(),
    createDiscordWebAdapter(),
  ],
});

export function createAutoRenewingOwnerPolicy({ now = () => new Date() } = {}) {
  const leases = new Map();
  return Object.freeze({
    profile: OWNER_PROFILE,
    decide(context) {
      const instant = now();
      const checkedAt = instant instanceof Date ? instant : new Date(instant);
      if (!Number.isFinite(checkedAt.getTime())) throw new TypeError('Owner policy clock returned an invalid date.');
      const key = `${context.origin}\u0000${context.capability}`;
      let lease = leases.get(key) ?? null;
      if (!lease || Date.parse(lease.expiresAt) <= checkedAt.getTime()) {
        lease = createLocalOwnerLease({
          now: checkedAt,
          ttlMs: MAX_LEASE_TTL_MS,
          origins: [context.origin],
          capabilities: [context.capability],
        });
        leases.set(key, lease);
      }
      return createOwnerAutonomyPolicy({
        profile: OWNER_PROFILE,
        lease,
        now: () => checkedAt,
      }).decide(context);
    },
  });
}

function fail(code, message, details = {}) {
  return { ok: false, error: { code, message, details }, provenance: PROVENANCE };
}

function validTabId(tabId) {
  return Number.isInteger(tabId) && tabId >= 0;
}

function validFrameId(frameId) {
  return Number.isInteger(frameId) && frameId >= 0;
}

function safeFailure(error, fallbackCode = 'EXTENSION_RUNTIME_FAILED', fallbackMessage = 'ToolBraid rejected the request safely.') {
  const code = typeof error?.code === 'string' && SAFE_ERROR_CODE.test(error.code)
    ? error.code
    : fallbackCode;
  const message = typeof error?.message === 'string' && error.message.length <= 320
    ? error.message
    : fallbackMessage;
  return fail(code, message, {});
}

function exactOrigin(value) {
  try {
    const url = new URL(value);
    if (!['http:', 'https:'].includes(url.protocol) || url.username || url.password) return null;
    return url.origin;
  } catch {
    return null;
  }
}

function bytesToBase64(bytes) {
  if (!(bytes instanceof Uint8Array) || typeof globalThis.btoa !== 'function') {
    throw new ProtocolError('VISUAL_ENCODER_UNAVAILABLE', 'Visible screenshot encoding is unavailable.');
  }
  let binary = '';
  for (let offset = 0; offset < bytes.length; offset += 0x8000) {
    binary += String.fromCharCode(...bytes.subarray(offset, Math.min(bytes.length, offset + 0x8000)));
  }
  return globalThis.btoa(binary);
}

function handoffSecret(cryptoRef) {
  if (!cryptoRef || typeof cryptoRef.getRandomValues !== 'function') {
    throw new ProtocolError('CRYPTO_UNAVAILABLE', 'Secure handoff key generation is unavailable.');
  }
  const bytes = new Uint8Array(32);
  cryptoRef.getRandomValues(bytes);
  return [...bytes].map((value) => value.toString(16).padStart(2, '0')).join('');
}

function handoffIntent(state, intent) {
  return syntheticUiIntent({
    handoffId: state.handoffId,
    type: state.type,
    missionId: state.missionId,
    memberId: state.memberId,
    sessionId: state.sessionId,
    pageFingerprint: state.pageFingerprint,
    targetFingerprint: state.targetFingerprint,
    purpose: state.purpose,
    safeOrigin: state.safeOrigin,
    intent,
  });
}

function handoffProof(state) {
  return {
    kind: 'toolbraid.completion-proof',
    fresh: true,
    handoffId: state.handoffId,
    type: state.type,
    binding: {
      missionId: state.missionId,
      memberId: state.memberId,
      sessionId: state.sessionId,
      pageFingerprint: state.pageFingerprint,
      targetFingerprint: state.targetFingerprint,
      purpose: state.purpose,
      safeOrigin: state.safeOrigin,
    },
  };
}

function extensionPageSender(chromeApi, sender, expectedPath = null) {
  const runtimeId = chromeApi?.runtime?.id;
  if (typeof runtimeId !== 'string' || sender?.id !== runtimeId || sender?.tab) return false;
  try {
    const url = new URL(sender.url);
    if (url.protocol !== 'chrome-extension:'
      || url.hostname !== runtimeId
      || url.port !== ''
      || url.username !== ''
      || url.password !== '') return false;
    return expectedPath === null || url.pathname === `/${expectedPath}`;
  } catch {
    return false;
  }
}

function runtimeBridgeSender(chromeApi, sender) {
  return extensionPageSender(chromeApi, sender, 'bridge.html');
}

function runtimeUiSender(chromeApi, sender) {
  return extensionPageSender(chromeApi, sender, 'sidepanel.html');
}

function pageRuntimeSender(chromeApi, sender) {
  const runtimeId = chromeApi?.runtime?.id;
  const tabId = sender?.tab?.id;
  const frameId = sender?.frameId ?? 0;
  if (typeof runtimeId !== 'string' || sender?.id !== runtimeId
    || !validTabId(tabId) || !validFrameId(frameId)) return false;
  return isInjectableUrl(sender?.url ?? sender?.tab?.url ?? '');
}

function targetUrl(sender, message) {
  return sender?.url ?? sender?.tab?.url ?? message?.url ?? '';
}

function crossesOrigin(registry, tabId, changeInfo, tab) {
  const nextUrl = typeof changeInfo?.url === 'string'
    ? changeInfo.url
    : (typeof tab?.pendingUrl === 'string' ? tab.pendingUrl : null);
  if (!nextUrl) return false;
  const currentUrl = registry.get(tabId, 0)?.url;
  if (typeof currentUrl !== 'string') return false;
  try {
    return new URL(currentUrl).origin !== new URL(nextUrl).origin;
  } catch {
    return true;
  }
}

export function createServiceWorkerController({
  chromeApi,
  registry = new TabLifecycleRegistry(),
  sendToContentScript = null,
  executeHandler = null,
  cryptoRef = globalThis.crypto,
  publicRelease = PUBLIC_RELEASE,
  onAccessChanged = () => {},
} = {}) {
  if (!chromeApi) throw new TypeError('chromeApi is required.');
  let accessEnabled = !publicRelease;
  const accessReady = publicRelease
    ? Promise.resolve().then(() => chromeApi.storage.local.get(PUBLIC_ACCESS_KEY)).then((stored) => {
      const access = stored?.[PUBLIC_ACCESS_KEY];
      accessEnabled = access?.version === 1 && access.enabled === true;
    }).catch(() => { accessEnabled = false; })
    : Promise.resolve();
  async function requireAccess() {
    await accessReady;
    if (!accessEnabled) throw new ProtocolError('ACCESS_PAUSED', 'AI control is paused. Enable it in the ToolBraid side panel.');
  }
  const sendUnchecked = sendToContentScript ?? (async (tabId, message, { frameId = 0 } = {}) => {
    if (!chromeApi.tabs?.sendMessage) throw new ProtocolError('CONTENT_SCRIPT_UNAVAILABLE', 'Chrome tabs.sendMessage is unavailable.');
    return chromeApi.tabs.sendMessage(tabId, message, { frameId });
  });
  const send = async (...args) => {
    await requireAccess();
    return sendUnchecked(...args);
  };
  const bridge = createUniversalBridge({ registry, sendToContentScript: send, executeHandler });
  let universalPromise = null;
  let missionPromise = null;
  let handoffPromise = null;
  const surfaceLifecycle = new Map();
  const handoffSurfaces = new Map();
  const captchaAttemptPromises = new Map();
  const reactivationOrigins = new Map();
  const reactivationPromises = new Map();
  const ownerPolicy = createAutoRenewingOwnerPolicy();
  const ownerAuthorizationPolicy = {
    profile: ownerPolicy.profile,
    decide(context) {
      if (!accessEnabled) throw new ProtocolError('ACCESS_PAUSED', 'AI control is paused.');
      return ownerPolicy.decide(context);
    },
  };
  const lifecycleForHandoff = Object.freeze({
    get(tabId, frameId = 0) {
      return surfaceLifecycle.get(`${tabId}:${frameId}`) ?? registry.get(tabId, frameId);
    },
  });

  function ensureUniversalRuntime() {
    if (!universalPromise) {
      universalPromise = createExtensionUniversalRuntime({
        chromeApi,
        registry,
        bridge,
        sendToContentScript: send,
        postconditionAdapterRegistry: POSTCONDITION_ADAPTERS,
        ownerAuthorizationPolicy,
      }).catch((error) => {
        universalPromise = null;
        throw error;
      });
    }
    return universalPromise;
  }

  function ensureMissionRuntime() {
    if (!missionPromise) {
      missionPromise = ensureUniversalRuntime()
        .then((universal) => createExtensionMissionRuntime({
          lifecycleRegistry: registry,
          universalRuntime: universal.runtime,
          store: createChromeStorageAdapter(chromeApi.storage.local),
        }))
        .catch((error) => {
          missionPromise = null;
          throw error;
        });
    }
    return missionPromise;
  }

  function ensureHandoffRuntime() {
    if (!handoffPromise) {
      handoffPromise = ensureMissionRuntime()
        .then(async (mission) => {
          const storageArea = chromeApi.storage.session ?? chromeApi.storage.local;
          const keyStore = createChromeStorageAdapter(storageArea);
          let persistenceKey = await keyStore.get(HANDOFF_KEY_STORAGE_KEY);
          if (persistenceKey === undefined) {
            persistenceKey = handoffSecret(cryptoRef);
            await keyStore.set(HANDOFF_KEY_STORAGE_KEY, persistenceKey);
          }
          if (typeof persistenceKey !== 'string' || !/^[a-f0-9]{64}$/.test(persistenceKey)) {
            throw new ProtocolError('HANDOFF_KEY_INVALID', 'The extension handoff key is invalid.');
          }
          return createExtensionHandoffRuntime({
            storageArea,
            persistenceKey,
            mission,
            lifecycle: lifecycleForHandoff,
            validateMissionBinding: (binding) => mission.validateBinding(binding),
            validateUiIntent: (token, expected) => token?.kind === 'toolbraid.synthetic-ui-intent'
              && token?.handoffId === expected.handoffId
              && token?.type === expected.type
              && token?.intent === expected.intent
              && token?.missionId === expected.missionId
              && token?.memberId === expected.memberId
              && token?.sessionId === expected.sessionId
              && token?.pageFingerprint === expected.pageFingerprint
              && token?.targetFingerprint === expected.targetFingerprint
              && token?.purpose === expected.purpose
              && token?.safeOrigin === expected.safeOrigin,
            validateCompletionProof: (proof, expected) => proof?.kind === 'toolbraid.completion-proof'
              && proof?.fresh === true
              && proof?.handoffId === expected.handoffId
              && proof?.type === expected.type
              && proof?.binding?.missionId === expected.missionId
              && proof?.binding?.memberId === expected.memberId
              && proof?.binding?.sessionId === expected.sessionId
              && proof?.binding?.pageFingerprint === expected.pageFingerprint
              && proof?.binding?.targetFingerprint === expected.targetFingerprint
              && proof?.binding?.purpose === expected.purpose
              && proof?.binding?.safeOrigin === expected.safeOrigin,
          });
        })
        .catch((error) => {
          handoffPromise = null;
          throw error;
        });
    }
    return handoffPromise;
  }

  function closeUniversalSessions(sessions, reason) {
    if (!universalPromise || !sessions?.length) return;
    void universalPromise.then((universal) => Promise.allSettled(
      sessions.map((session) => universal.closeSession(session, reason)),
    ));
  }

  async function browserTab(tabId) {
    if (!validTabId(tabId) || typeof chromeApi.tabs?.get !== 'function') {
      throw new ProtocolError('HANDOFF_SURFACE_UNAVAILABLE', 'The human handoff tab is unavailable.');
    }
    const tab = await chromeApi.tabs.get(tabId);
    if (!validTabId(tab?.id) || tab.id !== tabId || !Number.isInteger(tab?.windowId)) {
      throw new ProtocolError('HANDOFF_SURFACE_UNAVAILABLE', 'The human handoff tab is unavailable.');
    }
    return tab;
  }

  async function canonicalHandoffSurface(state, surfaceTabId) {
    const mission = await ensureMissionRuntime();
    const source = mission.getBinding(state.missionId, state.memberId);
    const session = registry.get(source.tabId, source.frameId);
    if (!session
      || session.sessionId !== state.sessionId
      || source.sessionId !== state.sessionId
      || source.pageFingerprint !== state.pageFingerprint
      || source.origin !== state.safeOrigin) {
      throw new ProtocolError('HANDOFF_SOURCE_DRIFT', 'The source page session is no longer current.');
    }
    const [sourceTab, surfaceTab] = await Promise.all([
      browserTab(source.tabId),
      browserTab(surfaceTabId),
    ]);
    const surfaceOrigin = exactOrigin(surfaceTab.pendingUrl ?? surfaceTab.url);
    if (!surfaceOrigin || surfaceOrigin !== state.safeOrigin) {
      throw new ProtocolError('HANDOFF_SURFACE_ORIGIN_MISMATCH', 'The human handoff tab is not on the exact approved origin.');
    }
    const binding = {
      missionId: state.missionId,
      memberId: state.memberId,
      sessionId: state.sessionId,
      pageFingerprint: state.pageFingerprint,
      targetFingerprint: state.targetFingerprint,
      purpose: state.purpose,
      safeOrigin: state.safeOrigin,
      tabId: source.tabId,
      frameId: source.frameId,
      windowId: sourceTab.windowId,
      documentId: session.documentId ?? null,
      pageInstanceId: session.pageInstanceId ?? null,
      origin: source.origin,
    };
    const surface = Object.freeze({
      kind: HANDOFF_SURFACE_KIND,
      createdBy: HANDOFF_SURFACE_CREATOR,
      surfaceId: `surface-${surfaceTab.id}-${state.handoffId.slice(0, 24)}`,
      tabId: surfaceTab.id,
      frameId: 0,
      windowId: surfaceTab.windowId,
      origin: surfaceOrigin,
      binding: Object.freeze(binding),
    });
    surfaceLifecycle.set(`${surface.tabId}:${surface.frameId}`, Object.freeze({ ...surface, state: 'active' }));
    handoffSurfaces.set(state.handoffId, surface);
    return surface;
  }

  async function refreshHandoffSurface(handoffId) {
    const surface = handoffSurfaces.get(handoffId);
    if (!surface) throw new ProtocolError('HANDOFF_SURFACE_REQUIRED', 'Open the human handoff again from the side panel.');
    const tab = await browserTab(surface.tabId);
    const origin = exactOrigin(tab.pendingUrl ?? tab.url);
    if (tab.windowId !== surface.windowId || origin !== surface.origin) {
      surfaceLifecycle.delete(`${surface.tabId}:${surface.frameId}`);
      handoffSurfaces.delete(handoffId);
      throw new ProtocolError('HANDOFF_SURFACE_DRIFT', 'The human handoff tab changed or closed.');
    }
    surfaceLifecycle.set(`${surface.tabId}:${surface.frameId}`, Object.freeze({ ...surface, state: 'active' }));
    return surface;
  }

  async function clickCaptchaCheckbox(surface) {
    if (!chromeApi.scripting || typeof chromeApi.scripting.executeScript !== 'function') {
      throw new ProtocolError('CAPTCHA_EXECUTION_UNAVAILABLE', 'The CAPTCHA surface execution API is unavailable.');
    }
    let results;
    try {
      results = await chromeApi.scripting.executeScript({
        target: { tabId: surface.tabId, frameIds: [0] },
        func: clickVisibleCaptchaCheckbox,
        world: 'ISOLATED',
      });
    } catch {
      throw new ProtocolError('CAPTCHA_EXECUTION_FAILED', 'The bounded CAPTCHA checkbox click could not be dispatched.');
    }
    const result = Array.isArray(results) ? results[0]?.result : results?.result;
    if (result?.ok !== true || result?.clicked !== true) {
      throw new ProtocolError(
        result?.error?.code ?? 'CAPTCHA_CHECKBOX_TARGET_INVALID',
        result?.error?.message ?? 'Exactly one visible top-frame CAPTCHA checkbox is required; no click was dispatched.',
      );
    }
    return result;
  }

  async function handleHandoffUiMessage(type, payload = {}) {
    const handoff = await ensureHandoffRuntime();
    if (type === HANDOFF_UI_MESSAGE_TYPES.GET_STATE) {
      return { ok: true, state: { handoffs: handoff.list() }, provenance: PROVENANCE };
    }
    if (type === HANDOFF_UI_MESSAGE_TYPES.REQUEST) {
      const mission = await ensureMissionRuntime();
      const binding = mission.getBinding(payload.missionId, payload.memberId);
      const sourceTab = await browserTab(binding.tabId);
      const state = await handoff.request({
        ...(typeof payload.handoffId === 'string' ? { handoffId: payload.handoffId } : {}),
        ...(Number.isFinite(payload.ttlMs) ? { ttlMs: payload.ttlMs } : {}),
        type: payload.type,
        missionId: binding.missionId,
        memberId: binding.memberId,
        sessionId: binding.sessionId,
        pageFingerprint: binding.pageFingerprint,
        targetFingerprint: payload.targetFingerprint ?? binding.pageFingerprint,
        purpose: payload.purpose,
        tabId: binding.tabId,
        frameId: binding.frameId,
        windowId: sourceTab.windowId,
        origin: binding.origin,
        safeOrigin: binding.origin,
      });
      return { ok: true, result: state, provenance: PROVENANCE };
    }
    if (type === HANDOFF_UI_MESSAGE_TYPES.OPEN_SURFACE) {
      const handoffId = payload.handoffId;
      let state = handoff.state(handoffId);
      const surface = await canonicalHandoffSurface(state, payload.surfaceTabId);
      if (state.state === HANDOFF_STATES.AWAITING_UI_GESTURE) {
        state = await handoff.open({ handoffId, uiIntent: handoffIntent(state, 'open') });
      }
      if (state.state === HANDOFF_STATES.OPENING) {
        state = await handoff.commit({ handoffId, surface });
      }
      if (state.state !== HANDOFF_STATES.HUMAN_ACTIVE) {
        throw new ProtocolError('HANDOFF_STATE_INVALID', 'The human handoff could not become active.');
      }
      return { ok: true, result: state, provenance: PROVENANCE };
    }
    if (type === HANDOFF_UI_MESSAGE_TYPES.CAPTCHA_ATTEMPT) {
      const handoffId = payload.handoffId;
      const state = handoff.state(handoffId);
      if (state.type !== 'captcha') {
        throw new ProtocolError('CAPTCHA_TYPE_REQUIRED', 'Checkbox attempts are only valid for CAPTCHA handoffs.');
      }
      if (state.state !== HANDOFF_STATES.HUMAN_ACTIVE) {
        throw new ProtocolError('HANDOFF_STATE_INVALID', 'The CAPTCHA handoff must be active in its exact human surface.');
      }
      if (state.captchaCheckboxAttempts >= 1) {
        throw new ProtocolError('CAPTCHA_ATTEMPT_LIMIT', 'Only one CAPTCHA checkbox attempt is permitted.');
      }
      const existing = captchaAttemptPromises.get(handoffId);
      if (existing) return existing;
      const attempt = (async () => {
        const currentSurface = await refreshHandoffSurface(handoffId);
        // Re-check the source mission binding immediately before the browser
        // operation; the broker's post-click validation must not be the first
        // place a source-page drift is discovered.
        const surface = await canonicalHandoffSurface(state, currentSurface.tabId);
        // The browser click is deliberately completed before the broker ledger
        // consumes the one allowed attempt. A missing/ambiguous target leaves
        // the human handoff active and never burns the attempt.
        await clickCaptchaCheckbox(surface);
        const attempted = await handoff.captchaCheckboxAttempt({
          handoffId,
          surface,
          uiIntent: handoffIntent(state, 'captcha-checkbox'),
        });
        return { ok: true, result: attempted, provenance: PROVENANCE };
      })();
      captchaAttemptPromises.set(handoffId, attempt);
      try {
        return await attempt;
      } finally {
        captchaAttemptPromises.delete(handoffId);
      }
    }
    if (type === HANDOFF_UI_MESSAGE_TYPES.COMPLETE) {
      const handoffId = payload.handoffId;
      const inFlightCaptcha = captchaAttemptPromises.get(handoffId);
      if (inFlightCaptcha) await inFlightCaptcha.catch(() => undefined);
      const state = handoff.state(handoffId);
      const surface = await refreshHandoffSurface(handoffId);
      const completed = await handoff.return({
        handoffId,
        surface,
        uiIntent: handoffIntent(state, 'complete'),
        completionProof: handoffProof(state),
      });
      surfaceLifecycle.delete(`${surface.tabId}:${surface.frameId}`);
      handoffSurfaces.delete(handoffId);
      return { ok: true, result: completed, provenance: PROVENANCE };
    }
    throw new ProtocolError('HANDOFF_UI_MESSAGE_INVALID', 'Unsupported handoff side-panel message type.');
  }

  function actionBinding(value) {
    if (!value || typeof value !== 'object' || Array.isArray(value)) return null;
    const actionId = value.actionId ?? value.id;
    if (typeof actionId !== 'string') return null;
    return {
      actionId,
      tabId: value.tabId,
      frameId: value.frameId,
      sessionId: value.sessionId,
      origin: value.origin,
      pageFingerprint: value.pageFingerprint,
      ...(value.documentId === undefined ? {} : { documentId: value.documentId }),
      ...(value.pageInstanceId === undefined ? {} : { pageInstanceId: value.pageInstanceId }),
    };
  }

  async function bindPreparedActionToMission(preparedAction) {
    const binding = actionBinding(preparedAction);
    if (!binding) return { status: 'unbound', reason: 'ACTION_BINDING_INVALID' };
    let mission;
    try {
      mission = await ensureMissionRuntime();
      const owner = mission.findOwnerByBinding(binding);
      if (!owner) return { status: 'unbound', reason: 'NO_EXACT_MISSION_OWNER' };
      await mission.registerPendingAction({
        missionId: owner.missionId,
        memberId: owner.memberId,
        actionId: binding.actionId,
      });
      return {
        status: 'bound',
        missionId: owner.missionId,
        memberId: owner.memberId,
        actionId: binding.actionId,
      };
    } catch (error) {
      return {
        status: 'unbound',
        reason: typeof error?.code === 'string' ? error.code : 'MISSION_LINK_FAILED',
      };
    }
  }

  async function resolveMissionAction(value) {
    const binding = actionBinding(value);
    if (!binding) return { status: 'unresolved', reason: 'ACTION_BINDING_INVALID' };
    try {
      const mission = await ensureMissionRuntime();
      const owner = mission.findOwnerByBinding(binding);
      if (!owner) return { status: 'unresolved', reason: 'NO_EXACT_MISSION_OWNER' };
      const result = await mission.resolvePendingAction({
        missionId: owner.missionId,
        memberId: owner.memberId,
        actionId: binding.actionId,
      });
      return {
        status: result?.resolvedActionIds?.includes(binding.actionId) ? 'resolved' : 'already-resolved',
        missionId: owner.missionId,
        memberId: owner.memberId,
        actionId: binding.actionId,
      };
    } catch (error) {
      return {
        status: 'unresolved',
        reason: typeof error?.code === 'string' ? error.code : 'MISSION_RESOLUTION_FAILED',
      };
    }
  }

  async function handlePageReady(message, sender) {
    if (!pageRuntimeSender(chromeApi, sender)) return fail('PAGE_SENDER_INVALID', 'Page readiness must originate from an extension content script in an HTTP(S) tab.');
    const tabId = sender?.tab?.id;
    const frameId = sender?.frameId ?? 0;
    if (!validTabId(tabId) || !validFrameId(frameId)) return fail('PAGE_SENDER_INVALID', 'Page readiness must originate from a tab frame.');
    const url = targetUrl(sender, message);
    if (!isInjectableUrl(url)) return fail('URL_NOT_INJECTABLE', 'ToolBraid only runs on HTTP(S) pages.');
    if (message.pageInstanceId !== undefined && (typeof message.pageInstanceId !== 'string' || message.pageInstanceId.length < 16 || message.pageInstanceId.length > 256)) {
      return fail('PAGE_INSTANCE_INVALID', 'The page instance binding is invalid.');
    }
    const previous = registry.get(tabId, frameId);
    const accepted = registry.acceptPageReady(tabId, {
      frameId,
      documentId: sender?.documentId ?? null,
      pageInstanceId: message.pageInstanceId ?? null,
      url,
    });
    if (frameId === 0) reactivationOrigins.set(tabId, exactOrigin(url));
    if (!accepted.reused && previous) closeUniversalSessions([previous], 'document-replaced');
    const channel = createEnvelope({
      type: MESSAGE_TYPES.CHANNEL_INIT,
      ...accepted.session,
      payload: { provenance: PROVENANCE },
    });
    return {
      ok: true,
      channel,
      reused: accepted.reused,
      provenance: PROVENANCE,
    };
  }

  async function handleBridgeRegistration(message, sender) {
    if (!runtimeBridgeSender(chromeApi, sender)) return fail('BRIDGE_SENDER_INVALID', 'Tool registration is accepted only from this extension.');
    const tabId = message?.tabId ?? message?.targetTabId;
    const frameId = message?.frameId ?? 0;
    return bridge.registerGeneratedTools({ tabId, frameId, sessionId: message?.sessionId, tools: message?.tools });
  }

  async function handlePageSnapshot(message, sender) {
    if (!pageRuntimeSender(chromeApi, sender)) return fail('PAGE_SENDER_INVALID', 'Snapshots must originate from an extension content script in an HTTP(S) tab.');
    const tabId = sender?.tab?.id;
    const frameId = sender?.frameId ?? 0;
    if (!validTabId(tabId) || !validFrameId(frameId)) return fail('PAGE_SENDER_INVALID', 'Snapshots must originate from a tab content script.');
    const session = registry.get(tabId, frameId);
    if (!session) return fail('SESSION_NOT_FOUND', 'The page session is no longer active.');
    if (message?.sessionId !== session.sessionId || message?.nonce !== session.nonce) {
      return fail('BINDING_MISMATCH', 'Snapshot binding did not match the active tab session.');
    }
    try {
      const universal = await ensureUniversalRuntime();
      const state = await universal.ingestPageSnapshot(message, sender);
      mcpEndpoint.pageContentChanged();
      if (missionPromise) {
        await missionPromise
          .then((mission) => mission.handlePageSnapshot({ tabId, frameId }, sender))
          .catch(() => undefined);
      }
      return { ok: true, state, provenance: PROVENANCE };
    } catch (error) {
      const normalized = asExtensionRuntimeError(error);
      return fail(normalized.code, normalized.message, normalized.details);
    }
  }

  async function withMissionAndHandoffState(response) {
    if (response?.ok !== true) return response;
    const [missionResult, handoffResult] = await Promise.allSettled([
      ensureMissionRuntime().then((mission) => mission.list()),
      ensureHandoffRuntime().then((handoff) => handoff.list()),
    ]);
    return {
      ...response,
      state: {
        ...response.state,
        missions: missionResult.status === 'fulfilled' ? missionResult.value : [],
        handoffs: handoffResult.status === 'fulfilled' ? handoffResult.value : [],
        ...(missionResult.status === 'rejected' ? { missionError: safeFailure(missionResult.reason).error } : {}),
        ...(handoffResult.status === 'rejected' ? { handoffError: safeFailure(handoffResult.reason).error } : {}),
      },
    };
  }

  async function handleUiMessage(message, sender) {
    if (!runtimeUiSender(chromeApi, sender)) return fail('UI_SENDER_INVALID', 'Side-panel requests are accepted only from the canonical ToolBraid side-panel.');
    try {
      if (isMissionUiMessageType(message.type)) {
        const mission = await ensureMissionRuntime();
        return await mission.handleUiMessage(message.type, message.payload ?? {});
      }
      if (isHandoffUiMessageType(message.type)) {
        return await handleHandoffUiMessage(message.type, message.payload ?? {});
      }
      const universal = await ensureUniversalRuntime();
      if (message.type === UI_MESSAGE_TYPES.UI_PREPARE_ACTION) {
        const response = await universal.handleUiMessage(message.type, message.payload ?? {});
        if (response?.ok === true && response.result?.status === 'approval-required' && response.preparedAction) {
          const missionBinding = await bindPreparedActionToMission(response.preparedAction);
          return { ...response, missionBinding };
        }
        return response;
      }
      const actionForCleanup = (message.type === UI_MESSAGE_TYPES.UI_APPROVE_ACTION && message.payload?.decision === 'deny')
        || message.type === UI_MESSAGE_TYPES.UI_EXECUTE_ACTION
        ? actionBinding(message.payload?.action ?? message.payload?.approval?.scope)
        : null;
      let response;
      try {
        response = await universal.handleUiMessage(message.type, message.payload ?? {});
      } catch (error) {
        if (message.type === UI_MESSAGE_TYPES.UI_EXECUTE_ACTION && error?.code === 'ACTION_OUTCOME_UNKNOWN' && actionForCleanup) {
          await resolveMissionAction(actionForCleanup);
        }
        throw error;
      }
      let missionBinding = null;
      if (actionForCleanup) missionBinding = await resolveMissionAction(actionForCleanup);
      if (missionBinding) return { ...response, missionBinding };
      if (message.type !== 'UI_GET_STATE' || response?.ok !== true) return response;
      return withMissionAndHandoffState(response);
    } catch (error) {
      return safeFailure(error);
    }
  }

  async function handleRuntimeMessage(message, sender = {}) {
    try {
      if (!message || typeof message !== 'object' || Array.isArray(message)) return fail('MESSAGE_INVALID', 'ToolBraid runtime message must be an object.');
      if (['UI_GET_ACCESS', 'UI_SET_ACCESS', 'UI_CONNECT_SITE'].includes(message.type)) {
        if (!runtimeUiSender(chromeApi, sender)) return fail('UI_SENDER_INVALID', 'Only the ToolBraid side panel can change AI control.');
        await accessReady;
        if (message.type === 'UI_SET_ACCESS') {
          if (!publicRelease || typeof message.payload?.enabled !== 'boolean') return fail('ACCESS_SETTING_INVALID', 'A public-edition control choice is required.');
          const enabled = message.payload.enabled;
          if (!enabled) {
            accessEnabled = false;
            onAccessChanged(false);
            reactivationOrigins.clear();
            closeUniversalSessions(registry.clear(), 'access-paused');
            mcpEndpoint.invalidate();
          }
          await chromeApi.storage.local.set({ [PUBLIC_ACCESS_KEY]: { version: 1, enabled } });
          accessEnabled = enabled;
          if (enabled) onAccessChanged(true);
        }
        const [tab] = await chromeApi.tabs.query({ active: true, currentWindow: true });
        if (message.type === 'UI_CONNECT_SITE') {
          await requireAccess();
          if (!tab || tab.id !== message.payload?.targetTabId || exactOrigin(tab.url) !== message.payload?.origin) return fail('ACTIVE_TAB_CHANGED', 'The active site changed. Review it and try again.');
          if (!await hasOwnerHostGrant(tab.url)) return fail('HOST_PERMISSION_REQUIRED', 'Allow access to this site before connecting.');
          return activateTab(tab);
        }
        return { ok: true, access: { enabled: accessEnabled, publicRelease }, tab: tab && isInjectableUrl(tab.url) ? { id: tab.id, origin: exactOrigin(tab.url) } : null };
      }
      await requireAccess();
      if (message.type === 'UI_COMMUNITY' || message.type === 'UI_WEBMCP') {
        if (!runtimeUiSender(chromeApi, sender)) return fail('UI_SENDER_INVALID', 'Only the ToolBraid side panel can use these controls.');
        if (message.type === 'UI_WEBMCP') return { ok: true, result: await webMcpOwnerTools.call('toolbraid.webmcp.discover') };
        const operations = { state: () => communityTools.state(), inspect: () => communityTools.inspectSelected(), watch: () => communityTools.watch(message.payload), pause: () => communityTools.pause() };
        if (!Object.hasOwn(operations, message.operation)) return fail('MESSAGE_INVALID', 'Unknown community operation.');
        return { ok: true, result: await operations[message.operation]() };
      }
      if (message.type === MESSAGE_TYPES.PAGE_READY) return handlePageReady(message, sender);
      if (message.type === MESSAGE_TYPES.PAGE_SNAPSHOT) return handlePageSnapshot(message, sender);
      if (isUiMessageType(message.type)
          || isMissionUiMessageType(message.type)
          || isHandoffUiMessageType(message.type)) return handleUiMessage(message, sender);
      if (message.type === MESSAGE_TYPES.BRIDGE_REGISTER_TOOLS) return handleBridgeRegistration(message, sender);
      if (message.type === MESSAGE_TYPES.PAGE_EVENT) {
        if (!pageRuntimeSender(chromeApi, sender)) return fail('PAGE_SENDER_INVALID', 'Page events must originate from an extension content script in an HTTP(S) tab.');
        const result = await bridge.handlePageEnvelope(message.envelope, sender);
        if (result?.envelope) return result;
        return result;
      }
      return fail('MESSAGE_TYPE_UNEXPECTED', 'ToolBraid runtime message type is not accepted.');
    } catch (error) {
      return safeFailure(error, 'MESSAGE_INVALID', 'ToolBraid runtime message could not be processed safely.');
    }
  }

  async function mcpGetState(payload = {}) {
    const universal = await ensureUniversalRuntime();
    const selectedTarget = Number.isInteger(payload?.targetFrameId)
      ? null
      : frameOwnerTools.selectedTarget();
    return withMissionAndHandoffState(await universal.handleUiMessage(
      UI_MESSAGE_TYPES.UI_GET_STATE,
      { ...payload, ...(selectedTarget ?? {}), refresh: true },
    ));
  }

  async function mcpExecuteRead(payload = {}) {
    const universal = await ensureUniversalRuntime();
    return universal.handleUiMessage(UI_MESSAGE_TYPES.UI_EXECUTE_READ, payload);
  }

  async function mcpPrepareAction(payload = {}) {
    const universal = await ensureUniversalRuntime();
    const response = await universal.handleUiMessage(UI_MESSAGE_TYPES.UI_PREPARE_ACTION, payload);
    if (response?.ok === true && response.result?.status === 'approval-required' && response.preparedAction) {
      const missionBinding = await bindPreparedActionToMission(response.preparedAction);
      return { ...response, missionBinding };
    }
    return response;
  }

  async function mcpExecuteOwnerAction(payload = {}) {
    const universal = await ensureUniversalRuntime();
    return universal.handleUiMessage(UI_MESSAGE_TYPES.UI_EXECUTE_OWNER_ACTION, {
      targetTabId: payload.targetTabId,
      targetWindowId: payload.targetWindowId,
      targetFrameId: payload.targetFrameId,
      sessionId: payload.sessionId,
      toolName: payload.toolName,
      arguments: payload.arguments,
    });
  }

  async function executeFileTargetMethod(method, request) {
    const results = await chromeApi.scripting.executeScript({
      target: { tabId: request.binding.tabId, frameIds: [request.binding.frameId] },
      world: 'ISOLATED',
      func: (methodName, payload) => {
        const executor = globalThis.ToolBraidUniversalActionExecutor ?? globalThis.ToolBraidUniversal?.actionExecutor;
        try {
          if (!executor || typeof executor[methodName] !== 'function') {
            return { ok: false, error: { code: 'FILE_TARGET_EXECUTOR_UNAVAILABLE', message: 'The isolated file target executor is unavailable.' } };
          }
          return executor[methodName]({ ...payload, documentRef: globalThis.document });
        } catch (error) {
          return { ok: false, error: { code: error?.code ?? 'FILE_TARGET_EXECUTION_FAILED', message: error?.message ?? 'The file target operation failed.' } };
        }
      },
      args: [method, request],
    });
    const result = Array.isArray(results) ? results[0]?.result : results?.result;
    if (!result || result.ok !== true) {
      throw new ProtocolError(result?.error?.code ?? 'FILE_TARGET_EXECUTION_FAILED', result?.error?.message ?? 'The isolated file target operation failed.');
    }
    return result;
  }

  async function attachGrantedFile(payload) {
    const sourceBinding = payload?.binding ?? payload;
    const binding = Object.freeze({
      tabId: sourceBinding?.tabId,
      windowId: sourceBinding?.windowId,
      frameId: sourceBinding?.frameId,
      sessionId: sourceBinding?.sessionId,
      origin: sourceBinding?.origin,
      pageFingerprint: sourceBinding?.pageFingerprint,
      extractorPageFingerprint: payload?.extractorPageFingerprint,
    });
    const liveSession = registry.get(binding.tabId, binding.frameId);
    if (!liveSession || liveSession.sessionId !== binding.sessionId || exactOrigin(liveSession.url) !== binding.origin) {
      throw new ProtocolError('MCP_FILE_BINDING_DRIFT', 'The file target page session changed before upload.');
    }
    const stateResponse = await mcpGetState({
      targetTabId: binding.tabId,
      targetWindowId: binding.windowId,
      targetFrameId: binding.frameId,
    });
    const state = stateResponse?.state;
    if (stateResponse?.ok !== true || state?.sessionId !== binding.sessionId
      || state?.tab?.id !== binding.tabId || state?.tab?.windowId !== binding.windowId
      || state?.tab?.frameId !== binding.frameId
      || exactOrigin(state?.tab?.origin) !== binding.origin
      || state?.snapshot?.pageFingerprint !== binding.pageFingerprint
      || typeof binding.extractorPageFingerprint !== 'string'
      || !/^[a-f0-9]{64}$/.test(binding.extractorPageFingerprint)
      || state?.snapshot?.extractorPageFingerprint !== binding.extractorPageFingerprint) {
      throw new ProtocolError('MCP_FILE_BINDING_DRIFT', 'The file target page binding changed before upload.');
    }
    const controller = createFileUploadController({ chromeApi, randomSource: cryptoRef });
    const common = {
      binding,
      ref: payload.fileInput.ref,
      pageFingerprint: binding.extractorPageFingerprint,
      expectedAccept: payload.fileInput.accept,
      expectedMultiple: payload.fileInput.multiple,
    };
    const result = await controller.upload({
      binding,
      targetRef: payload.fileInput.ref,
      resolvedLocalPath: payload.resolvedLocalPath,
      expectedFile: payload.expectedFile,
      markTarget: ({ marker, attribute }) => executeFileTargetMethod('markFileTarget', { ...common, marker, attribute }),
      revalidateTarget: ({ marker, attribute }) => executeFileTargetMethod('revalidateFileTarget', { ...common, marker, attribute }),
      verifyTarget: async ({ marker, attribute }) => {
        const verified = await executeFileTargetMethod('verifyFileTarget', { ...common, marker, attribute });
        const file = Array.isArray(verified.files) && verified.files.length === 1 ? verified.files[0] : null;
        return { ok: true, file: file ? { name: file.name, size: file.size, count: verified.count } : null };
      },
      cleanupTarget: ({ marker, attribute }) => executeFileTargetMethod('cleanupFileTarget', {
        binding, ref: payload.fileInput.ref, pageFingerprint: binding.extractorPageFingerprint, marker, attribute,
      }),
    });
    return { ok: result.ok === true, file: result.file };
  }

  async function mcpSnapshotContext() {
    const stateResponse = await mcpGetState({});
    const state = stateResponse?.state;
    if (stateResponse?.ok !== true || !state?.tab || typeof state.sessionId !== 'string') {
      throw new ProtocolError('MCP_CONTEXT_INVALID', 'The selected browser frame is unavailable.');
    }
    const frameId = state.tab.frameId ?? 0;
    const universal = await ensureUniversalRuntime();
    const rawSnapshot = await universal.refreshSnapshot(state.tab.id, frameId, state.sessionId);
    const frames = [];
    if (frameId === 0) {
      for (const session of registry.list(state.tab.id).filter((entry) => entry.frameId !== 0).slice(0, 32)) {
        try {
          const frameSnapshot = await universal.refreshSnapshot(session.tabId, session.frameId, session.sessionId);
          frames.push(Object.freeze({
            accessible: true,
            frameId: session.frameId,
            url: session.url,
            name: null,
            snapshot: Object.freeze({ ...frameSnapshot, pageFingerprint: universal.state(session.tabId, session.frameId).pageFingerprint }),
          }));
        } catch { /* a drifting frame is not represented as accessible */ }
      }
    }
    return Object.freeze({
      binding: Object.freeze({
        tabId: state.tab.id,
        windowId: state.tab.windowId,
        frameId,
        sessionId: state.sessionId,
        origin: state.tab.origin,
        pageFingerprint: state.snapshot.pageFingerprint,
      }),
      snapshot: Object.freeze({ ...rawSnapshot, pageFingerprint: state.snapshot.pageFingerprint, frames: Object.freeze(frames) }),
    });
  }

  function sameVisualBinding(left, right) {
    return Boolean(left && right
      && left.tabId === right.tabId
      && left.windowId === right.windowId
      && left.frameId === right.frameId
      && left.sessionId === right.sessionId
      && left.origin === right.origin
      && left.pageFingerprint === right.pageFingerprint);
  }

  async function visualViewport(context) {
    const results = await chromeApi.scripting.executeScript({
      target: { tabId: context.binding.tabId, frameIds: [0] },
      world: 'ISOLATED',
      func: () => ({
        width: Math.max(1, Math.floor(globalThis.innerWidth || globalThis.document?.documentElement?.clientWidth || 0)),
        height: Math.max(1, Math.floor(globalThis.innerHeight || globalThis.document?.documentElement?.clientHeight || 0)),
        deviceScaleFactor: Math.max(1, Math.min(8, Math.round(globalThis.devicePixelRatio || 1))),
        origin: globalThis.location?.origin ?? null,
      }),
    });
    const viewport = Array.isArray(results) ? results[0]?.result : results?.result;
    if (!viewport || viewport.origin !== context.binding.origin
      || !Number.isInteger(viewport.width) || !Number.isInteger(viewport.height)
      || !Number.isInteger(viewport.deviceScaleFactor)) {
      throw new ProtocolError('VISUAL_VIEWPORT_INVALID', 'The visible viewport changed before capture.');
    }
    return Object.freeze({ width: viewport.width, height: viewport.height, deviceScaleFactor: viewport.deviceScaleFactor });
  }

  async function captureVisual({ context, objective = '', analyze = true }) {
    if (context.binding.frameId !== 0) throw new ProtocolError('VISUAL_TOP_FRAME_REQUIRED', 'Visual capture requires the selected top frame.');
    const before = await mcpSnapshotContext();
    if (!sameVisualBinding(before.binding, context.binding)) throw new ProtocolError('VISUAL_CONTEXT_DRIFT', 'The selected page changed before visual capture.');
    const universal = await ensureUniversalRuntime();
    const viewport = await visualViewport(context);
    const screenshot = await universal.browserCapture.captureVisibleScreenshot({
      windowId: context.binding.windowId,
      locationRef: { href: context.snapshot.metadata.url, origin: context.binding.origin },
    });
    const asset = screenshot?.asset ?? screenshot;
    const entry = asset?.handle ? universal.browserCapture.handleStore?.get?.(asset.handle) : null;
    if (!entry?.bytes) {
      if (asset?.handle) universal.browserCapture.handleStore?.release?.(asset.handle);
      throw new ProtocolError('VISUAL_CAPTURE_INVALID', 'The visible screenshot could not be retained for analysis.');
    }
    const bytes = new Uint8Array(entry.bytes);
    try {
      const after = await mcpSnapshotContext();
      if (!sameVisualBinding(after.binding, context.binding)) throw new ProtocolError('VISUAL_CONTEXT_DRIFT', 'The selected page changed during visual capture.');
      let analysis = null;
      if (analyze) {
        const configuration = await universal.multimodalSettings.read();
        if (!configuration.enabled || !configuration.visionModel) {
          throw new ProtocolError('MULTIMODAL_PROVIDER_REQUIRED', 'Configure an OpenAI-compatible multimodal provider before visual inspection.');
        }
        const prompt = [
          'Analyze this visible browser screenshot for the owner-controlled automation task.',
          `Viewport is ${viewport.width}x${viewport.height} CSS pixels at device scale ${viewport.deviceScaleFactor}.`,
          'Return JSON regions for relevant visible controls. Each region should include label, role, x, y, width, height, and confidence in CSS-pixel coordinates.',
          'Do not identify or solve CAPTCHA, human-verification, security-code, password, or payment targets.',
          objective ? `Owner objective: ${objective}` : 'Owner objective: describe actionable visible controls.',
        ].join(' ');
        const adapter = createOpenAiCompatibleVisionAdapter({
          id: 'toolbraid-visual-targeting',
          version: '1',
          baseUrl: configuration.baseUrl,
          model: configuration.visionModel,
          getApiKey: async () => configuration.apiKey,
          resolveImage: async () => ({ base64: bytesToBase64(bytes), mimeType: asset.mimeType || 'image/png' }),
          prompt,
        });
        analysis = await adapter.analyze({ ...asset, kind: 'image' }, { context: { pageOrigin: context.binding.origin } });
      }
      return Object.freeze({ bytes, viewport, analysis });
    } catch (error) {
      bytes.fill(0);
      throw error;
    } finally {
      if (asset?.handle) universal.browserCapture.handleStore?.release?.(asset.handle);
      try { entry.bytes.fill(0); } catch { /* copied entry only */ }
    }
  }

  async function dispatchVisualClick({ context, point, descriptor }) {
    const current = await mcpSnapshotContext();
    if (!sameVisualBinding(current.binding, context.binding)) throw new ProtocolError('VISUAL_CONTEXT_DRIFT', 'The selected page changed before visual input.');
    const expectedHash = descriptor?.screenshotHash;
    const guard = await captureVisual({ context, objective: '', analyze: false });
    let guardHash = null;
    try {
      const digest = new Uint8Array(await cryptoRef.subtle.digest('SHA-256', guard.bytes));
      guardHash = [...digest].map((value) => value.toString(16).padStart(2, '0')).join('');
    } finally {
      try { guard.bytes.fill(0); } catch { /* best effort */ }
    }
    if (guardHash !== expectedHash) throw new ProtocolError('VISUAL_TARGET_DRIFT', 'The visible pixels changed immediately before input dispatch.');
    const debuggee = { tabId: context.binding.tabId };
    let attached = false;
    try {
      await chromeApi.debugger.attach(debuggee, '1.3');
      attached = true;
      await chromeApi.debugger.sendCommand(debuggee, 'Input.dispatchMouseEvent', { type: 'mouseMoved', x: point.x, y: point.y });
      await chromeApi.debugger.sendCommand(debuggee, 'Input.dispatchMouseEvent', { type: 'mousePressed', x: point.x, y: point.y, button: 'left', clickCount: 1 });
      await chromeApi.debugger.sendCommand(debuggee, 'Input.dispatchMouseEvent', { type: 'mouseReleased', x: point.x, y: point.y, button: 'left', clickCount: 1 });
    } catch {
      throw new ProtocolError('VISUAL_CLICK_FAILED', 'The exact visual click could not be dispatched.');
    } finally {
      if (attached) {
        try { await chromeApi.debugger.detach(debuggee); } catch { /* debugger lifecycle is already ending */ }
      }
    }
  }

  async function inspectAccessibilityTree({ context }) {
    if (context.binding.frameId !== 0) throw new ProtocolError('AX_TOP_FRAME_REQUIRED', 'Accessibility fallback requires the selected top frame.');
    const current = await mcpSnapshotContext();
    if (!sameVisualBinding(current.binding, context.binding)) throw new ProtocolError('AX_CONTEXT_DRIFT', 'The selected page changed before accessibility inspection.');
    const debuggee = { tabId: context.binding.tabId };
    let attached = false;
    try {
      await chromeApi.debugger.attach(debuggee, '1.3');
      attached = true;
      await chromeApi.debugger.sendCommand(debuggee, 'Accessibility.enable');
      const result = await chromeApi.debugger.sendCommand(debuggee, 'Accessibility.getFullAXTree', { depth: 12 });
      const after = await mcpSnapshotContext();
      if (!sameVisualBinding(after.binding, context.binding)) throw new ProtocolError('AX_CONTEXT_DRIFT', 'The selected page changed during accessibility inspection.');
      return Object.freeze({ nodes: Object.freeze(Array.isArray(result?.nodes) ? result.nodes.slice(0, 512) : []) });
    } catch (error) {
      if (error instanceof ProtocolError) throw error;
      throw new ProtocolError('AX_INSPECTION_FAILED', 'The accessibility tree could not be inspected.');
    } finally {
      if (attached) {
        try { await chromeApi.debugger.detach(debuggee); } catch { /* debugger lifecycle is already ending */ }
      }
    }
  }

  async function activateAccessibilityNode({ context, node }) {
    const current = await mcpSnapshotContext();
    if (!sameVisualBinding(current.binding, context.binding)) throw new ProtocolError('AX_CONTEXT_DRIFT', 'The selected page changed before accessibility activation.');
    const debuggee = { tabId: context.binding.tabId };
    let attached = false;
    try {
      await chromeApi.debugger.attach(debuggee, '1.3');
      attached = true;
      await chromeApi.debugger.sendCommand(debuggee, 'DOM.enable');
      const model = await chromeApi.debugger.sendCommand(debuggee, 'DOM.getBoxModel', { backendNodeId: node.backendDOMNodeId });
      const quad = model?.model?.border ?? model?.model?.content;
      if (!Array.isArray(quad) || quad.length !== 8 || quad.some((value) => !Number.isFinite(value))) {
        throw new ProtocolError('AX_NODE_NOT_VISIBLE', 'The exact accessibility node has no visible box.');
      }
      const x = Math.round((quad[0] + quad[2] + quad[4] + quad[6]) / 4);
      const y = Math.round((quad[1] + quad[3] + quad[5] + quad[7]) / 4);
      await chromeApi.debugger.sendCommand(debuggee, 'Input.dispatchMouseEvent', { type: 'mouseMoved', x, y });
      await chromeApi.debugger.sendCommand(debuggee, 'Input.dispatchMouseEvent', { type: 'mousePressed', x, y, button: 'left', clickCount: 1 });
      await chromeApi.debugger.sendCommand(debuggee, 'Input.dispatchMouseEvent', { type: 'mouseReleased', x, y, button: 'left', clickCount: 1 });
      return Object.freeze({ point: Object.freeze({ x, y }), backendNodeId: node.backendDOMNodeId });
    } catch (error) {
      if (error instanceof ProtocolError) throw error;
      throw new ProtocolError('AX_ACTIVATION_FAILED', 'The exact accessibility control could not be activated.');
    } finally {
      if (attached) {
        try { await chromeApi.debugger.detach(debuggee); } catch { /* debugger lifecycle is already ending */ }
      }
    }
  }

  function researchUrl(value) {
    try {
      const parsed = new URL(value);
      if (!['http:', 'https:'].includes(parsed.protocol) || parsed.username || parsed.password) return null;
      parsed.hash = '';
      return parsed.href;
    } catch { return null; }
  }

  async function captureResearchSnapshot(binding) {
    let tab;
    try { tab = await chromeApi.tabs.get(binding.tabId); } catch { throw new ProtocolError('RESEARCH_TAB_DRIFT', 'The attached research tab is unavailable.'); }
    if (tab?.windowId !== binding.windowId || researchUrl(tab?.url) !== researchUrl(binding.url)) {
      throw new ProtocolError('RESEARCH_TAB_DRIFT', 'The attached research tab identity or URL changed.');
    }
    const session = registry.get(binding.tabId, 0);
    if (!session || session.state !== 'active' || researchUrl(session.url) !== researchUrl(binding.url)) {
      throw new ProtocolError('RESEARCH_TAB_SESSION_MISSING', 'Activate ToolBraid on the research tab before capturing it.');
    }
    const universal = await ensureUniversalRuntime();
    const snapshot = await universal.refreshSnapshot(session.tabId, 0, session.sessionId);
    const state = universal.state(session.tabId, 0);
    if (!state || state.sessionId !== session.sessionId || typeof state.pageFingerprint !== 'string') {
      throw new ProtocolError('RESEARCH_SNAPSHOT_DRIFT', 'The research page changed during capture.');
    }
    return Object.freeze({
      ...snapshot,
      metadata: Object.freeze({ ...snapshot.metadata, url: tab.url }),
      pageFingerprint: state.pageFingerprint,
    });
  }

  const browserOwnerTools = createBrowserOwnerTools({ chromeApi });
  let mcpEndpoint = null;
  const frameOwnerTools = createFrameOwnerTools({
    chromeApi,
    lifecycleRegistry: registry,
    cryptoRef,
    onSelectionChanged: () => mcpEndpoint?.invalidatePage(),
  });
  const humanVerificationOwnerTools = createHumanVerificationOwnerTools({
    getSnapshotContext: mcpSnapshotContext,
    cryptoRef,
  });
  const visualOwnerTools = createVisualOwnerTools({
    captureVisual,
    getSnapshotContext: mcpSnapshotContext,
    dispatchClick: dispatchVisualClick,
    cryptoRef,
  });
  const resilienceOwnerTools = createResilienceOwnerTools({
    getSnapshotContext: mcpSnapshotContext,
    waitRef: (milliseconds) => new Promise((resolve) => globalThis.setTimeout(resolve, milliseconds)),
  });
  const accessibilityOwnerTools = createAccessibilityOwnerTools({
    getSnapshotContext: mcpSnapshotContext,
    inspectTree: inspectAccessibilityTree,
    activateNode: activateAccessibilityNode,
    cryptoRef,
  });
  const researchOwnerTools = createResearchOwnerTools({
    resolveTabHandle: (handle) => browserOwnerTools.resolveHandle(handle),
    captureSnapshot: captureResearchSnapshot,
    cryptoRef,
  });
  const memoryKeyValueStore = chromeApi.storage?.local
    ? createChromeStorageAdapter(chromeApi.storage.local)
    : null;
  const memoryOwnerTools = createMemoryOwnerTools({
    storage: {
      async load() { return memoryKeyValueStore ? (await memoryKeyValueStore.get(MEMORY_STORAGE_KEY) ?? []) : []; },
      async save(records) {
        if (!memoryKeyValueStore) throw new ProtocolError('MEMORY_STORAGE_UNAVAILABLE', 'Local extension storage is unavailable.');
        await memoryKeyValueStore.set(MEMORY_STORAGE_KEY, records);
      },
    },
    authorizeMutation: ({ operation, arguments: mutationArguments }) => ownerAuthorizationPolicy.decide({
      classification: 'mutate',
      origin: 'toolbraid://local-memory',
      capability: `local.memory.${operation}`,
      arguments: mutationArguments,
    }),
    cryptoRef,
  });
  async function permittedTarget(context = {}) {
    await requireAccess();
    const response = await mcpGetState(context);
    if (!response?.ok || !response.state?.tab) throw new ProtocolError('CHAT_PAGE_UNAVAILABLE', 'Connect the selected site before sharing its tools with chat.');
    const { tab, sessionId } = response.state;
    if (!await hasOwnerHostGrant(tab.url)) throw new ProtocolError('HOST_PERMISSION_REQUIRED', 'Allow ToolBraid access to the selected site first.');
    return { tabId: tab.id, windowId: tab.windowId, frameId: tab.frameId ?? 0, url: tab.url, sessionId };
  }
  const webMcpOwnerTools = createWebMcpOwnerTools({ chromeApi, getTarget: permittedTarget, cryptoRef });
  const communityTools = createCommunityTools({ chromeApi, authorize: async (url) => {
    await requireAccess();
    if (!await hasOwnerHostGrant(url)) throw new ProtocolError('HOST_PERMISSION_REQUIRED', 'Connect this X site to ToolBraid before inspecting it.');
  } });
  const nativeOwnerToolProviders = [
    webMcpOwnerTools,
    communityTools,
    browserOwnerTools,
    frameOwnerTools,
    humanVerificationOwnerTools,
    visualOwnerTools,
    resilienceOwnerTools,
    accessibilityOwnerTools,
    researchOwnerTools,
    memoryOwnerTools,
  ];
  if (typeof chromeApi.downloads?.search === 'function') {
    nativeOwnerToolProviders.push(createDownloadOwnerTools({ chromeApi, cryptoRef }));
  }
  mcpEndpoint = createExtensionMcpEndpoint({
    authorize: requireAccess,
    getState: mcpGetState,
    executeRead: mcpExecuteRead,
    prepareAction: mcpPrepareAction,
    executeOwnerAction: mcpExecuteOwnerAction,
    attachGrantedFile,
    nativeOwnerToolProviders,
  });

  async function activateTab(tab) {
    const tabId = tab?.id;
    if (!validTabId(tabId)) return fail('TAB_ID_INVALID', 'An active tab id is required.');
    if (!isInjectableUrl(tab?.url ?? '')) return fail('URL_NOT_INJECTABLE', 'ToolBraid only runs on HTTP(S) pages.');
    const target = { tabId, allFrames: true };
    try {
      await requireAccess();
      // MAIN first: the content script's handshake cannot be lost before the
      // page-world listener exists. Both scripts are still scoped to the
      // user-activated tab and do not require host_permissions.
      await chromeApi.scripting.executeScript({
        target,
        files: ['protocol-runtime.js', 'injector-main.js'],
        world: 'MAIN',
        injectImmediately: true,
      });
      await chromeApi.scripting.executeScript({
        target,
        files: ['protocol-runtime.js', 'page-extractor.js', 'action-executor.js', 'rendered-media-capture.js', 'content-script.js'],
        world: 'ISOLATED',
        injectImmediately: true,
      });
      reactivationOrigins.set(tabId, exactOrigin(tab.url));
      return { ok: true, tabId, provenance: PROVENANCE };
    } catch (error) {
      return fail(error?.code ?? 'INJECTION_FAILED', error?.message ?? 'ToolBraid could not initialize the active tab.');
    }
  }

  async function requestOwnerHostGrant(tab) {
    const url = tab?.url ?? '';
    if (!isInjectableUrl(url)) return false;
    const origins = publicRelease ? [`${exactOrigin(url)}/*`] : url.startsWith('https:')
      ? ['https://*/*']
      : ['http://localhost/*', 'http://127.0.0.1/*'];
    if (!chromeApi.permissions?.request) return false;
    try {
      return await chromeApi.permissions.request({ origins }) === true;
    } catch {
      return false;
    }
  }

  async function hasOwnerHostGrant(url) {
    const origin = exactOrigin(url);
    if (!origin || !chromeApi.permissions?.contains) return false;
    try {
      return await chromeApi.permissions.contains({ origins: [`${origin}/*`] }) === true;
    } catch {
      return false;
    }
  }

  async function activateOwnerTab(tab) {
    if (publicRelease) {
      await accessReady;
      if (!accessEnabled) return fail('ACCESS_PAUSED', 'Enable AI control in the ToolBraid side panel.');
      if (!await hasOwnerHostGrant(tab?.url)) return fail('HOST_PERMISSION_REQUIRED', 'Connect this site from the ToolBraid side panel.');
      return activateTab(tab);
    }
    // permissions.request must begin directly inside the extension-action user
    // gesture. Chrome persists an accepted optional host grant for later
    // cross-origin owner navigation; declining it still permits this activeTab.
    await requestOwnerHostGrant(tab);
    return activateTab(tab);
  }

  async function handleActivatedTab(tabId) {
    if (publicRelease) {
      await accessReady;
      if (!accessEnabled) return fail('ACCESS_PAUSED', 'AI control is paused.');
    }
    if (!validTabId(tabId) || !chromeApi.tabs?.get) {
      return fail('TAB_ID_INVALID', 'An activated tab id is required.');
    }
    let tab;
    try {
      tab = await chromeApi.tabs.get(tabId);
    } catch (error) {
      return fail(error?.code ?? 'TAB_UNAVAILABLE', error?.message ?? 'The activated tab is unavailable.');
    }
    if (!isInjectableUrl(tab?.url ?? '')) return fail('URL_NOT_INJECTABLE', 'ToolBraid only runs on HTTP(S) pages.');
    if (!await hasOwnerHostGrant(tab.url)) {
      return fail('HOST_PERMISSION_REQUIRED', 'Persistent owner host access was not granted.');
    }
    reactivationOrigins.set(tabId, exactOrigin(tab.url));
    if (tab.status !== 'complete') return { ok: true, tabId, pending: true };
    if (reactivationPromises.has(tabId)) return reactivationPromises.get(tabId);
    const pending = activateTab(tab)
      .then((result) => {
        if (result?.ok !== true) reactivationOrigins.delete(tabId);
        return result;
      })
      .catch((error) => {
        reactivationOrigins.delete(tabId);
        return fail(error?.code ?? 'INJECTION_FAILED', error?.message ?? 'ToolBraid could not initialize the activated tab.');
      })
      .finally(() => reactivationPromises.delete(tabId));
    reactivationPromises.set(tabId, pending);
    return pending;
  }

  async function handleWindowFocused(windowId) {
    if (!chromeApi.tabs?.query) return fail('TAB_UNAVAILABLE', 'The focused window has no accessible active tab.');
    let tabs;
    try {
      tabs = await chromeApi.tabs.query({ active: true, windowId });
    } catch (error) {
      return fail(error?.code ?? 'TAB_UNAVAILABLE', error?.message ?? 'The focused window has no accessible active tab.');
    }
    return handleActivatedTab(tabs?.[0]?.id);
  }

  function reactivateTabAfterNavigation(tabId, tab) {
    if (reactivationPromises.has(tabId)) return;
    const expectedOrigin = reactivationOrigins.get(tabId);
    const actualOrigin = exactOrigin(tab?.url);
    if (!expectedOrigin || !actualOrigin) return;
    const pending = Promise.resolve(expectedOrigin === actualOrigin || hasOwnerHostGrant(tab?.url))
      .then((allowed) => allowed ? activateTab(tab) : fail('HOST_PERMISSION_REQUIRED', 'Persistent owner host access was not granted.'))
      .then((result) => {
        if (result?.ok === true) return;
        const invalidated = registry.invalidate(tabId, 'reactivation-failed');
        closeUniversalSessions(invalidated, 'reactivation-failed');
        reactivationOrigins.delete(tabId);
      })
      .catch(() => {
        const invalidated = registry.invalidate(tabId, 'reactivation-failed');
        closeUniversalSessions(invalidated, 'reactivation-failed');
        reactivationOrigins.delete(tabId);
      })
      .finally(() => reactivationPromises.delete(tabId));
    reactivationPromises.set(tabId, pending);
  }

  function handleTabUpdated(tabId, changeInfo = {}, tab = {}) {
    if (!validTabId(tabId)) return [];
    if (changeInfo.status === 'loading' || typeof changeInfo.url === 'string') {
      mcpEndpoint.invalidatePage();
      const currentOrigin = exactOrigin(registry.get(tabId, 0)?.url ?? tab.url);
      if (currentOrigin) reactivationOrigins.set(tabId, currentOrigin);
    }
    if (missionPromise) {
      void missionPromise
        .then((mission) => mission.handleTabUpdated(tabId, changeInfo, tab))
        .catch(() => undefined);
    }
    for (const [handoffId, surface] of handoffSurfaces.entries()) {
      if (surface.tabId !== tabId) continue;
      const nextOrigin = exactOrigin(changeInfo.url ?? tab.pendingUrl ?? tab.url);
      if (changeInfo.status === 'loading' && nextOrigin && nextOrigin !== surface.origin) {
        surfaceLifecycle.delete(`${surface.tabId}:${surface.frameId}`);
        handoffSurfaces.delete(handoffId);
      }
    }
    // Chromium reports History API transitions as `loading`, indistinguishable
    // from a same-origin document navigation in tabs.onUpdated. Cross-origin
    // changes can be closed immediately. Same-origin authority is replaced by
    // the definitive documentId/pageInstanceId boundary in PAGE_READY; until
    // then live snapshot and content-channel binding still fail closed.
    if (changeInfo.status === 'loading' && crossesOrigin(registry, tabId, changeInfo, tab)) {
      const invalidated = registry.invalidate(tabId, 'navigation');
      closeUniversalSessions(invalidated, 'navigation');
      return invalidated;
    }
    if (changeInfo.status === 'loading' && typeof changeInfo.url !== 'string') {
      const invalidated = registry.invalidate(tabId, 'reload');
      closeUniversalSessions(invalidated, 'reload');
      return invalidated;
    }
    if (changeInfo.status === 'complete') reactivateTabAfterNavigation(tabId, tab);
    return [];
  }

  function handleTabRemoved(tabId) {
    if (!validTabId(tabId)) return [];
    mcpEndpoint.invalidatePage();
    reactivationOrigins.delete(tabId);
    reactivationPromises.delete(tabId);
    if (missionPromise) {
      void missionPromise
        .then((mission) => mission.handleTabRemoved(tabId))
        .catch(() => undefined);
    }
    for (const [handoffId, surface] of handoffSurfaces.entries()) {
      if (surface.tabId !== tabId) continue;
      surfaceLifecycle.delete(`${surface.tabId}:${surface.frameId}`);
      handoffSurfaces.delete(handoffId);
    }
    const invalidated = registry.invalidate(tabId, 'tab-closed');
    closeUniversalSessions(invalidated, 'tab-closed');
    return invalidated;
  }

  function handleActiveContextChanged() {
    mcpEndpoint.invalidatePage();
  }

  return Object.freeze({
    registry,
    bridge,
    handleRuntimeMessage,
    activateTab,
    activateOwnerTab,
    handleActivatedTab,
    handleWindowFocused,
    handleTabUpdated,
    handleTabRemoved,
    handleActiveContextChanged,
    sessionBinding,
    ensureUniversalRuntime,
    ensureMissionRuntime,
    ensureHandoffRuntime,
    mcpEndpoint,
    accessReady,
    accessEnabled: () => accessEnabled,
    requireAccess,
    permittedTarget,
    communityTools,
  });
}

export function installServiceWorker(chromeApi = globalThis.chrome) {
  if (!chromeApi?.runtime?.onMessage) return null;
  let nativeMcpBridge = null;
  const assistantPorts = new Set();
  const controller = createServiceWorkerController({
    chromeApi,
    onAccessChanged: (enabled) => {
      nativeMcpBridge?.setEnabled(enabled);
      if (!enabled) void controller.communityTools.pause();
    },
  });
  nativeMcpBridge = installNativeMcpBridge({ chromeApi, endpoint: controller.mcpEndpoint, enabled: !PUBLIC_RELEASE,
    onAssistantEvent: (event) => {
      for (const port of assistantPorts) { try { port.postMessage(event); } catch { assistantPorts.delete(port); } }
    },
  });
  void controller.accessReady.then(() => nativeMcpBridge.setEnabled(controller.accessEnabled()));
  void controller.accessReady.then(() => controller.accessEnabled() ? controller.communityTools.restore() : controller.communityTools.pause()).catch(() => {});
  chromeApi.alarms?.onAlarm?.addListener((alarm) => { if (alarm.name === COMMUNITY_ALARM) void controller.communityTools.tick(); });
  chromeApi.runtime.onMessage.addListener((message, sender, sendResponse) => {
    if (message?.type === 'ASSISTANT_EVENT') return false;
    const dispatch = async () => {
      if (message?.type !== 'UI_ASSISTANT') return controller.handleRuntimeMessage(message, sender);
      if (!runtimeUiSender(chromeApi, sender)) return fail('UI_SENDER_INVALID', 'Only the ToolBraid side panel can control chat.');
      await controller.requireAccess();
      const params = { ...(message.payload ?? {}) };
      delete params.target;
      if (message.method === 'send' && params.usePage === true) {
        const target = await controller.permittedTarget();
        params.target = { targetTabId: target.tabId, targetWindowId: target.windowId, targetFrameId: target.frameId };
      }
      return { ok: true, result: await nativeMcpBridge.requestAssistant(message.method, params) };
    };
    Promise.resolve(dispatch())
      .then((response) => sendResponse(response))
      .catch((error) => sendResponse(errorPayload(error)));
    return true;
  });
  chromeApi.runtime.onConnect?.addListener((port) => {
    if (port?.name === 'toolbraid-assistant-panel') {
      if (!runtimeUiSender(chromeApi, port.sender)) { port.disconnect?.(); return; }
      assistantPorts.add(port);
      port.onDisconnect?.addListener(() => {
        void chromeApi.runtime.lastError;
        assistantPorts.delete(port);
      });
      return;
    }
    if (port?.name !== PAGE_LIFECYCLE_PORT) return;
    port.onDisconnect?.addListener(() => { void chromeApi.runtime.lastError; });
    if (!pageRuntimeSender(chromeApi, port.sender)) {
      try {
        port.disconnect?.();
      } catch { /* an invalid lifecycle port remains unauthoritative */ }
      return;
    }
    port.onMessage?.addListener((message) => {
      if (message?.type !== MESSAGE_TYPES.PAGE_READY) return;
      Promise.resolve(controller.handleRuntimeMessage(message, port.sender))
        .then((response) => port.postMessage?.(response))
        .catch((error) => port.postMessage?.(errorPayload(error)))
        .catch?.(() => {});
    });
  });
  chromeApi.action?.onClicked?.addListener((tab) => {
    void controller.activateOwnerTab(tab);
    nativeMcpBridge.connect();
    if (Number.isInteger(tab?.id)) {
      try {
        const opened = chromeApi.sidePanel?.open?.({ tabId: tab.id });
        opened?.catch?.(() => {});
      } catch { /* unsupported side panel remains non-fatal */ }
    }
  });
  chromeApi.tabs?.onUpdated?.addListener((tabId, changeInfo, tab) => {
    controller.handleTabUpdated(tabId, changeInfo, tab);
  });
  chromeApi.tabs?.onRemoved?.addListener((tabId) => {
    controller.handleTabRemoved(tabId);
  });
  chromeApi.tabs?.onActivated?.addListener((activeInfo) => {
    controller.handleActiveContextChanged();
    void controller.handleActivatedTab(activeInfo?.tabId);
  });
  chromeApi.windows?.onFocusChanged?.addListener((windowId) => {
    controller.handleActiveContextChanged();
    void controller.handleWindowFocused(windowId);
  });
  return controller;
}

const installedController = (typeof globalThis.chrome !== 'undefined')
  ? installServiceWorker(globalThis.chrome)
  : null;

export { installedController };
