...
test('keeps extension tools usable without native WebMCP and preserves approval checks', async () => {
  const executions = [];
  const runtime = createUniversalSessionRuntime({
    approvalLedger: await createPersistentApprovalLedger({ store: createMemoryKeyValueStore() }),
    registerTools: async () => ({ ok: false, error: { code: 'WEBMCP_UNSUPPORTED' } }),
    executePageAction: async (request) => { executions.push(request); return { changed: true }; },
  });
  const request = { tabId: 1, sessionId: 'without-native-webmcp' };
  const state = await runtime.ingest({ ...request, snapshot: snapshot() });
  assert.equal(state.toolTransport, 'extension-mcp');
  const read = state.tools.find((tool) => tool.sourceType === 'page');
  assert.equal((await runtime.executeTool({ ...request, name: read.name })).type, 'page');
  const mutation = state.tools.find((tool) => tool.target?.ref === 'draft');
  const property = Object.keys(mutation.inputSchema.properties)[0];
  const pending = await runtime.executeTool({ ...request, name: mutation.name, input: { [property]: 'Draft only' } });
  assert.equal(pending.status, 'approval-required');
  assert.equal(executions.length, 0);
  await assert.rejects(runtime.executeTool({ ...request, sessionId: 'wrong-session', name: read.name }), /session/i);
});

test('does not hide registration failures other than missing native WebMCP', async () => {
  for (const code of ['REGISTRATION_FAILED', 'BINDING_MISMATCH', 'CONTENT_SCRIPT_UNAVAILABLE']) {
    const runtime = createUniversalSessionRuntime({
      approvalLedger: await createPersistentApprovalLedger({ store: createMemoryKeyValueStore() }),
      registerTools: async () => ({ ok: false, error: { code } }),
      executePageAction: async () => { throw new Error('not expected'); },
    });
    await assert.rejects(
      runtime.ingest({ tabId: 1, sessionId: 'failed-registration', snapshot: snapshot() }),
      (error) => error.code === 'TOOL_REGISTRATION_FAILED',
    );
  }
});
