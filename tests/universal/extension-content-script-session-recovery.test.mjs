...
test('missing MAIN registration acknowledgement releases the channel and ignores a late response', () => {
  const h = makeHarness({ readyResponses: [{ binding: firstBinding, reused: false }] });
  const requestId = 'registration-timeout-123456';
  const request = makePageEnvelope(h.context, h.context.ToolBraidUniversalProtocol.TYPES.REGISTER_TOOLS,
    firstBinding, { tools: [] }, requestId);
  const responses = [];
  assert.equal(h.context.__tbRuntimeListener(request, {}, (response) => responses.push(response)), true);
  assert.equal(h.context.__TOOLBRAID_UNIVERSAL_CONTENT__.pendingResponses.size, 1);
  h.fireNext(3000);
  assert.equal(responses[0].error.code, 'MAIN_REGISTRATION_TIMEOUT');
  assert.equal(h.context.__TOOLBRAID_UNIVERSAL_CONTENT__.pendingResponses.size, 0);
  const late = makePageEnvelope(h.context, h.context.ToolBraidUniversalProtocol.TYPES.REGISTER_RESULT,
    firstBinding, { ok: true }, requestId);
  vm.runInContext('globalThis.__tbLateEvent = { source: window, origin: location.origin }', h.context);
  h.context.__tbLateEvent.data = late;
  h.context.__tbWindowListener(h.context.__tbLateEvent);
...
test('the lifecycle Port accepts only the exact extension content-script sender', async () => {
  ...
  const chromeApi = {
    storage: { local: {
      get: async () => ({ 'toolbraid.public.access.v1': { version: 1, enabled: true } }),
    ...
    runtime: {
      ...
      get lastError() { disconnectErrorsRead += 1; return { message: 'Page moved into back/forward cache.' }; },
    },
  ...
  connectListener({
    ...
    onDisconnect: { addListener(listener) { validDisconnectListener = listener; } },
  ...
  const errorsBeforeDisconnect = disconnectErrorsRead;
  validDisconnectListener();
  assert.equal(disconnectErrorsRead, errorsBeforeDisconnect + 1);
