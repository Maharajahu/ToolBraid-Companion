...
import { SiteAdapterError, createSiteAdapterRegistry } from '../../src/site-adapters/registry.js';
...
test('verified X adapter exposes a direct like but does not mislabel the closed repost menu button as completion', () => {
  ...
  assert.deepEqual(tools.map((tool) => tool.name), ['read_x_post', 'read_x_conversation', 'open_x_reply_composer', 'like_x_post', 'open_x_repost_menu']);
  assert.equal(tools.some((tool) => tool.name === 'repost_x_post'), false);
  assert.equal(tools[0].annotations.readOnlyHint, true);
  const like = tools.find((tool) => tool.name === 'like_x_post');
  assert.equal(like.requiresApproval, true);
  assert.equal(like.provenance.source, 'toolbraid.verified-adapter');
  assert.equal(like.provenance.pageFingerprint, snapshot.pageFingerprint);
});
...
test('verified X mutations reject unlike, undo repost, and already-active controls', () => {
  ...
  assert.deepEqual(names, ['read_x_post', 'read_x_conversation']);
});

test('verified X adapter prepares reply text as an owner-policy mutation only in an already-open editor', () => {
  const registry = createSiteAdapterRegistry({ adapters: [createXPostAdapter()] });
  const snapshot = xSnapshot({
    accessibleControls: [
  ...
  const prepared = registry.generateTools(snapshot).find((tool) => tool.name === 'prepare_x_reply');
  assert.ok(prepared);
  assert.equal(prepared.classification, 'mutate');
  assert.equal(prepared.target.ref, 'reply-editor');
  assert.match(prepared.description, /already-open X composer/);
});
...
test('live-derived Romanian X snapshot uses stable controls and reads only the target post', () => {
  ...
  const tools = registry.generateTools(snapshot);
  assert.deepEqual(tools.map((tool) => tool.name), ['read_x_post', 'read_x_conversation', 'prepare_x_reply', 'like_x_post', 'open_x_repost_menu']);
...
test('post controls stop at the exact article even when outside ancestors are truncated', () => {
  const registry = createSiteAdapterRegistry({ adapters: [createXPostAdapter()] });
  const snapshot = xSnapshot({
    links: [{ ref: 'permalink', href: 'https://x.com/Maharajahu/status/42', text: 'Today' }],
    accessibleControls: [
      { ref: 'like', role: 'button', name: 'Like', attributes: { 'data-testid': 'like' } },
      { ref: 'quote-like', role: 'button', name: 'Like', attributes: { 'data-testid': 'like' } },
      { ref: 'reply-like', role: 'button', name: 'Like', attributes: { 'data-testid': 'like' } },
    ],
    elementRefs: [
      { ref: 'post', tagName: 'article', parentRef: 'truncated-outside-ancestor' },
      { ref: 'permalink', tagName: 'a', parentRef: 'post', attributes: { 'data-timezone': 'Europe/London' } },
      { ref: 'like', tagName: 'button', parentRef: 'post', attributes: { 'data-testid': 'like' } },
      { ref: 'quote', tagName: 'article', parentRef: 'post' },
      { ref: 'quote-like', tagName: 'button', parentRef: 'quote', attributes: { 'data-testid': 'like' } },
      { ref: 'reply', tagName: 'article' },
      { ref: 'reply-like', tagName: 'button', parentRef: 'reply', attributes: { 'data-testid': 'like' } },
    ],
  });
  assert.equal(registry.generateTools(snapshot).find((tool) => tool.name === 'like_x_post')?.target.ref, 'like');
});
