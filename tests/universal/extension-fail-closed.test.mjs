...
function mockChrome({ hostGrant = false, requestGrant = hostGrant } = {}) {
  const calls = [];
  return {
    calls,
    api: {
      ...
      permissions: {
        request: async (details) => {
          calls.push({ operation: 'requestPermission', details });
          return requestGrant;
        },
        contains: async (details) => {
          calls.push({ operation: 'containsPermission', details });
          return hostGrant;
        },
      },
    },
...
test('activation refuses non-HTTP(S) pages and only then injects explicit worlds', async () => {
  ...
  assert.deepEqual(calls.map((call) => call.details.target), [{ tabId: 7, allFrames: true }, { tabId: 7, allFrames: true }]);
});

test('owner activation requests persistent HTTPS host access before injecting the active tab', async () => {
  const { api, calls } = mockChrome({ requestGrant: true });
  const controller = createServiceWorkerController({ publicRelease: false, chromeApi: api });

  const active = await controller.activateOwnerTab({ id: 7, url: PAGE });

  assert.equal(active.ok, true);
  assert.deepEqual(calls.map((call) => call.operation), ['requestPermission', 'executeScript', 'executeScript']);
  assert.deepEqual(calls[0].details, { origins: ['https://*/*'] });
});

test('a persistent owner host grant reactivates after cross-origin Google navigation', async () => {
  const { api, calls } = mockChrome({ hostGrant: true });
  const controller = createServiceWorkerController({ publicRelease: false, chromeApi: api });
  await ready(controller);
  const googleUrl = 'https://accounts.google.com/o/oauth2/auth';

  controller.handleTabUpdated(7, { status: 'loading', url: googleUrl }, { id: 7, url: googleUrl });
  controller.handleTabUpdated(7, { status: 'complete' }, { id: 7, url: googleUrl });
  await new Promise((resolve) => setImmediate(resolve));

  assert.deepEqual(
    calls.filter((call) => call.operation === 'containsPermission').map((call) => call.details),
...
test('cross-origin reactivation stays fail-closed when persistent host access is absent', async () => {
  const { api, calls } = mockChrome({ hostGrant: false });
  const controller = createServiceWorkerController({ publicRelease: false, chromeApi: api });
  await ready(controller);
  const googleUrl = 'https://accounts.google.com/o/oauth2/auth';
  ...
  assert.equal(calls.some((call) => call.operation === 'containsPermission'), true);
  assert.equal(calls.some((call) => call.operation === 'executeScript'), false);
  assert.equal(controller.registry.get(7), null);
});
