...
test('side panel permission surface is explicit and points to its default document', () => {
  ...
  assert.deepEqual(new Set(manifest.permissions), new Set(['activeTab', 'alarms', 'debugger', 'downloads', 'nativeMessaging', 'scripting', 'storage', 'sidePanel', 'tabs']));
...
test('advanced tools show granted permission state instead of requesting an unsupported optional permission', () => {
  assert.match(html, /<span id="access-debugger"[^>]*role="status"/);
  assert.doesNotMatch(html, /Allow advanced tools|<button id="access-debugger"/);
  assert.match(js, /browser\.permissions\.contains\(\{ permissions: \['debugger'\] \}\)/);
  assert.doesNotMatch(js, /permissions\.request\(\{ permissions: \['debugger'\]/);
});
...
test('side panel has extension CSP, no inline script, and renders through safe DOM APIs', () => {
  ...
  assert.match(html, /<title>ToolBraid<\/title>/);
  assert.match(html, /<h1>Browser tools<\/h1>/);
  assert.match(html, /id="owner-mode-badge"/);
  assert.match(html, /id="owner-control-plane"/);
  assert.match(html, /id="owner-model-route"/);
  assert.match(html, /CAPTCHA \/ MFA steps/);
  assert.match(html, /FALLBACK &amp; HISTORY/);
  assert.doesNotMatch(html, /HUMAN AUTHORITY/);
  ...
  assert.equal((html.match(/aria-live="polite"/g) ?? []).length, 3);
...
test('project support links stay optional, local-rendered and isolated from the extension', () => {
  const footer = html.match(/<footer class="project-footer">[\s\S]*?<\/footer>/)?.[0];
  assert.ok(footer);
  assert.match(footer, /Free to use/);
  assert.match(footer, /Support is optional/);
  const links = [...footer.matchAll(/<a\b([^>]+)>/g)].map((match) => match[1]);
  assert.equal(links.length, 2);
  assert.match(links[0], /href="https:\/\/buymeacoffee\.com\/dumitrescup"/);
  assert.match(links[1], /href="https:\/\/github\.com\/Maharajahu\/ToolBraid-Companion"/);
  ...
  assert.doesNotMatch(footer, /<(?:script|iframe|img)\b|\bonclick=/i);
  assert.match(css, /\.project-link:focus-visible/);
});

test('activates Owner Autonomy only for a ready connection with an explicit runtime enablement', () => {
  const defaults = normalizeState({});
  assert.deepEqual(defaults.ownerMode, {
    active: false,
    controlPlane: 'Local',
    modelRoute: 'Local / cloud via MCP',
    policy: 'owner-autonomous',
  });

  const disconnected = normalizeState({ ownerMode: {
    enabled: true,
    controlPlane: 'Local native bridge',
    modelRoute: 'Cloud model via MCP',
    profile: 'owner-autonomous',
  } });
  assert.deepEqual(disconnected.ownerMode, {
    active: false,
    controlPlane: 'Local native bridge',
    modelRoute: 'Cloud model via MCP',
    policy: 'owner-autonomous',
  ...
  const active = normalizeState({ connection: 'ready', ownerMode: {
    enabled: true,
    controlPlane: 'Local native bridge',
    modelRoute: 'Cloud model via MCP',
    profile: 'owner-autonomous',
  } });
  assert.deepEqual(active.ownerMode, {
    active: true,
    controlPlane: 'Local native bridge',
    modelRoute: 'Cloud model via MCP',
    policy: 'owner-autonomous',
...
test('controller emits the fail-closed terminal mission phase contract', async () => {
  ...
  const runtime = {
    sendMessage(message) {
      ...
      if (message.type === UI_MESSAGE_TYPES.UI_GET_STATE) return Promise.resolve({ ok: true, state: {
        ...
        tab: { id: 4, url: 'https://example.test/current' },
        snapshot: { pageFingerprint: 'a'.repeat(64) },
        missions: [{ missionId: 'mission-1', phase: 'running', revision: ++stateReads === 1 ? 7 : 8, members: [] }],
      } });
  ...
  assert.deepEqual(calls.at(-1), {
    type: UI_MESSAGE_TYPES.UI_MISSION_SET_PHASE,
    payload: { missionId: 'mission-1', phase: 'completed', expectedRevision: 8 },
  });
});
