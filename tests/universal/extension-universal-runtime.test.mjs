...
test('MCP viewport scroll dispatches without approval, refreshes tools and returns observed page text', async () => {
  let scrolled = false;
  const h = harness({ onSend: ({ message }) => {
    if (message.type === MESSAGE_TYPES.PAGE_ACTION_EXECUTE) {
      assert.equal(message.approved, false);
      assert.equal(message.preparedAction.effect.operation, 'scroll');
      scrolled = true;
      return { ok: true, receipt: { operation: 'scroll', before: { top: 0 }, after: { top: 600 }, moved: true, status: 'scrolled' } };
    }
    if (message.type === MESSAGE_TYPES.PAGE_EXTRACT_SNAPSHOT && scrolled) return { ok: true, snapshot: { ...rawSnapshot(), mainText: 'Newly loaded replies' } };
  } });
  const integration = await createExtensionUniversalRuntime({ chromeApi: h.chromeApi, registry: h.registry, bridge: h.bridge,
    sendToContentScript: h.sendToContentScript, store: createMemoryKeyValueStore(), localApprovalStore: h.localApprovalStore });
  const state = await integration.ingestPageSnapshot({ sessionId: h.session.sessionId, snapshot: rawSnapshot() }, { tab: { id: 7 }, frameId: 0 });
  const tool = state.tools.find((entry) => entry.name === 'universal_scroll_page');
  assert.ok(tool);
  const result = await h.executeHandler()({ tabId: 7, frameId: 0, sessionId: h.session.sessionId, name: tool.name, input: { direction: 'down' } });
  assert.equal(result.operation, 'scroll');
  assert.equal(result.mainText, 'Newly loaded replies');
  assert.notEqual(result.pageFingerprint, state.pageFingerprint);
  assert.equal(h.localApprovals.size, 0);
  assert.ok(h.registrations.length >= 2);
});
...
test('fresh MCP state observes changes before the content watcher and does not reingest an unchanged page', async () => {
  let current = rawSnapshot();
  let failRefresh = false;
  const h = harness({ onSend: ({ message }) => {
    if (message.type !== MESSAGE_TYPES.PAGE_EXTRACT_SNAPSHOT) return;
    if (failRefresh) throw new Error('Page is no longer reachable');
    return { ok: true, snapshot: current };
  } });
  const integration = await createExtensionUniversalRuntime({
    chromeApi: h.chromeApi, registry: h.registry, bridge: h.bridge,
    sendToContentScript: h.sendToContentScript, store: createMemoryKeyValueStore(),
    localApprovalStore: h.localApprovalStore,
  });
  await integration.ingestPageSnapshot({ sessionId: h.session.sessionId, snapshot: current }, { tab: { id: 7 }, frameId: 0 });
  const before = (await integration.handleUiMessage(UI_MESSAGE_TYPES.UI_GET_STATE)).state;
  current = { ...current, mainText: 'The like has been applied.' };
  const fresh = (await integration.handleUiMessage(UI_MESSAGE_TYPES.UI_GET_STATE, { refresh: true })).state;
  assert.notEqual(fresh.snapshot.pageFingerprint, before.snapshot.pageFingerprint);
  assert.equal(h.registrations.length, 2);
  const unchanged = (await integration.handleUiMessage(UI_MESSAGE_TYPES.UI_GET_STATE, { refresh: true })).state;
  assert.equal(unchanged.audit.count, fresh.audit.count);
  assert.equal(h.registrations.length, 2);
  current = { ...current, pageFingerprint: 'a'.repeat(64), fileInputs: [{ ref: 'upload', accept: 'image/*' }] };
  const upload = (await integration.handleUiMessage(UI_MESSAGE_TYPES.UI_GET_STATE, { refresh: true })).state;
  assert.equal(upload.snapshot.extractorPageFingerprint, current.pageFingerprint);
  assert.equal(upload.fileInputs[0].ref, 'upload');
  failRefresh = true;
  await assert.rejects(() => integration.handleUiMessage(UI_MESSAGE_TYPES.UI_GET_STATE, { refresh: true }));
...
test('routes owner execution through runtime policy without accepting caller approval authority', async () => {
  const h = harness();
  const integration = await createExtensionUniversalRuntime({
    chromeApi: h.chromeApi,
    registry: h.registry,
  ...
  const mutation = state.tools.find((tool) => tool.classification === 'mutate');
  const property = Object.keys(mutation.inputSchema.properties)[0];
  const response = await integration.handleUiMessage(UI_MESSAGE_TYPES.UI_EXECUTE_OWNER_ACTION, {
    sessionId: h.session.sessionId,
    toolName: mutation.name,
    arguments: { [property]: 'Owner draft' },
    approved: true,
  });
  assert.equal(response.result.status, 'approval-required');
  assert.equal(response.preparedAction.actionId, response.result.preparedAction.actionId);
  assert.equal(h.pageExecutions.length, 0);

  await assert.rejects(
    integration.handleUiMessage(UI_MESSAGE_TYPES.UI_EXECUTE_OWNER_ACTION, {
...
test('dispatches an owner action when the extension-owned policy authorizes it', async () => {
  const h = harness();
  const integration = await createExtensionUniversalRuntime({
    chromeApi: h.chromeApi,
    registry: h.registry,
    ...
    ownerAuthorizationPolicy: Object.freeze({
      decide: async () => Object.freeze({ decision: 'auto', reason: 'owner-lease', leaseId: 'extension-owner-lease' }),
    }),
  });
  const state = await integration.ingestPageSnapshot(
    { sessionId: h.session.sessionId, snapshot: rawSnapshot() },
  ...
  const response = await integration.handleUiMessage(UI_MESSAGE_TYPES.UI_EXECUTE_OWNER_ACTION, {
    sessionId: h.session.sessionId,
    toolName: mutation.name,
    arguments: { [property]: 'Abliterated owner action' },
  });
...
for (const observationMode of ['immediate', 'delayed-x', 'missing-x']) test(`observes a fresh page snapshot for a bound postcondition: ${observationMode}`, async () => {
  const h = harness();
  const adapterId = observationMode === 'immediate' ? 'fixture' : 'x-post';
  ...
  const descriptor = {
    ...generic,
    ...(observationMode !== 'immediate' ? { name: 'publish_x_post' } : {}),
  ...
  const postconditionAdapters = createSiteAdapterRegistry({
    adapters: [{
      ...
      verifyPostcondition: (context) => {
        ...
        observations += 1;
        if (observationMode === 'missing-x' || (observationMode === 'delayed-x' && observations < 3)) {
          return { status: 'unverified', reasonCode: 'X_PUBLICATION_NOT_CONFIRMED', afterPageFingerprint: context.afterSnapshot.pageFingerprint };
        }
        return {
  ...
  assert.equal(executed.result.status, observationMode === 'missing-x' ? 'dispatched' : 'verified-success');
  assert.equal(executed.result.postcondition, observationMode === 'missing-x' ? 'unverified' : 'satisfied');
  assert.equal(observations, observationMode === 'immediate' ? 1 : observationMode === 'delayed-x' ? 3 : 8);
