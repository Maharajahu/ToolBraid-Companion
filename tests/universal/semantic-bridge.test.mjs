...
test('generic GET forms, navigation, and value changes require approval and the tool surface is bounded', () => {
  ...
  const scroll = tools.find((tool) => tool.name === 'universal_scroll_page');
  assert.equal(scroll.classification, 'read');
  assert.equal(scroll.requiresApproval, false);
  assert.ok(tools.slice(1).filter((tool) => tool !== scroll).every((tool) => tool.classification === 'mutate' && tool.requiresApproval));
