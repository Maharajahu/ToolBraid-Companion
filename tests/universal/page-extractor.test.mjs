...
test('classic extractor exposes pathless metadata for visible file inputs', () => {
  const html = new FakeNode('html');
  const body = new FakeNode('body');
  const label = new FakeNode('label', { for: 'media-file' }, 'Choose media');
  const input = new FakeNode('input', {
    id: 'media-file', type: 'file', accept: 'image/*,video/*', multiple: true, value: 'C:\\private\\secret.mp4',
  });
  const disabled = new FakeNode('input', { id: 'disabled-file', type: 'file', disabled: true, 'aria-label': 'Disabled upload' });
  label.append(input);
  body.append(label, disabled);
  html.append(body);
  const snapshot = loadExtractor().ToolBraidUniversalPageExtractor.extract({ documentRef: new FakeDocument(html) });
  assert.deepEqual(JSON.parse(JSON.stringify(snapshot.fileInputs)), [{
    ref: snapshot.fileInputs[0].ref,
    name: 'Choose media',
    accept: 'image/*,video/*',
    multiple: true,
  }]);
  assert.equal(JSON.stringify(snapshot.fileInputs).includes('secret.mp4'), false);
  assert.equal(snapshot.fileInputs.some(({ name }) => name === 'Disabled upload'), false);
});

test('classic extractor only advertises genuinely editable contenteditable targets', () => {
  const context = loadExtractor();
  const html = new FakeNode('html');
  const body = new FakeNode('body');
  const locked = new FakeNode('div', { id: 'locked', contenteditable: 'false', 'aria-label': 'Locked note' }, 'Keep');
  const rich = new FakeNode('div', { id: 'rich', contenteditable: 'true', 'aria-label': 'Rich note' }, 'Edit');
  const plain = new FakeNode('div', { id: 'plain', contenteditable: 'plaintext-only', 'aria-label': 'Plain note' }, 'Edit');
  const inherited = new FakeNode('div', { id: 'inherited', 'aria-label': 'Inherited note' }, 'Edit');
  inherited.isContentEditable = true;
  body.append(locked, rich, plain, inherited);
  html.append(body);

  const snapshot = context.ToolBraidUniversalPageExtractor.extract({ documentRef: new FakeDocument(html) });
  const controlNames = Array.from(snapshot.accessibleControls, (control) => control.name);

  assert.ok(!controlNames.includes('Locked note'));
  assert.deepEqual(controlNames, ['Rich note', 'Plain note', 'Inherited note']);
  assert.equal(snapshot.elementRefs.find((element) => element.ref === 'id:locked').role, null);
  for (const id of ['rich', 'plain', 'inherited']) {
    assert.equal(snapshot.elementRefs.find((element) => element.ref === `id:${id}`).role, 'textbox');
  }
});
