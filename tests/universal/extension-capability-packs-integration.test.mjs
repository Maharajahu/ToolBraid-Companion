...
test('extension resolves every personal social and messaging pack lazily and exposes redacted state', async () => {
  const cases = [
    [pageSnapshot('https://x.com/alice/status/123'), 'site.x', 'read_x_post'],
    [pageSnapshot('https://www.youtube.com/watch?v=abc'), 'site.youtube', 'read_youtube_content'],
    [pageSnapshot('https://www.instagram.com/p/abc/'), 'site.instagram', 'read_instagram_content'],
    [pageSnapshot('https://www.reddit.com/r/test/comments/abc/post/'), 'site.reddit', 'read_reddit_content'],
    [pageSnapshot('https://accounts.google.com/AccountChooser', {
      accessibleControls: [{ ref: 'owner-account', role: 'button', name: 'owner@example.com' }],
      elementRefs: [{ ref: 'owner-account', tagName: 'button', role: 'button', name: 'owner@example.com' }],
    }), 'site.google-account', 'choose_google_account'],
    [pageSnapshot('https://studio.tiktok.com/upload', {
      accessibleControls: [{ ref: 'caption', role: 'textbox', name: 'Caption' }],
      elementRefs: [{ ref: 'caption', tagName: 'textarea', role: 'textbox', name: 'Caption' }],
    }), 'site.tiktok', 'prepare_tiktok_caption'],
    [pageSnapshot('https://web.whatsapp.com/', {
      accessibleControls: [{ ref: 'wa-editor', role: 'textbox', name: 'Type a message' }],
      elementRefs: [{ ref: 'wa-editor', tagName: 'div', role: 'textbox', name: 'Type a message' }],
    }), 'site.whatsapp', 'read_whatsapp_conversation'],
    [pageSnapshot('https://web.telegram.org/a/', {
      accessibleControls: [{ ref: 'tg-editor', role: 'textbox', name: 'Message' }],
      elementRefs: [{ ref: 'tg-editor', tagName: 'div', role: 'textbox', name: 'Message' }],
    }), 'site.telegram', 'read_telegram_conversation'],
    [pageSnapshot('https://discord.com/channels/@me/123', {
      accessibleControls: [{ ref: 'dc-editor', role: 'textbox', name: 'Message #general' }],
      elementRefs: [{ ref: 'dc-editor', tagName: 'div', role: 'textbox', name: 'Message #general' }],
    }), 'site.discord', 'read_discord_channel'],
  ];
  for (const [snapshot, expectedPackId, expectedToolName] of cases) {
    const url = snapshot.metadata.url;
    ...
    assert.equal(tools.some((tool) => tool.name === expectedToolName), true, `${expectedPackId} should expose ${expectedToolName}; got ${tools.map((tool) => tool.name).join(', ')}`);
...
test('capability-pack tool surface is bounded, unique, and page fields cannot install a pack or alter the trusted catalog', async () => {
  ...
  const tools = packTools(state);
  assert.equal(state.tools.length, 32);
  assert.equal(tools.length, 30); // Reserve page read and viewport scrolling within the existing cap.
  assert.equal(new Set(tools.map((tool) => tool.name)).size, 30);
  assert.ok(state.tools.some((tool) => tool.name === 'universal_scroll_page'));
