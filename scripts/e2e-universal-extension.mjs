...
const DEFAULT_EXTENSION_DIR = path.join(PROJECT_ROOT, 'dist', 'toolbraid-extension');
...
async function prepareLaunchBundle({ sourceExtensionDir, sourceManifest, fixtureOrigin, hostOrigins = null }) {
  ...
  if (!sourceManifest.permissions?.includes('debugger') || sourceManifest.optional_permissions?.includes('debugger')) {
    fail('E2E_SOURCE_MANIFEST_AUTHORITY', 'The production manifest must declare debugger as required, not optional.', {
      sourceExtensionDir,
  ...
  if (!Array.isArray(requestedOrigins)) {
    fail('E2E_HOST_ORIGIN_INVALID', 'Exact launch origins must be an array.');
  }
...
async function createDebuggerExtensionPage(worker, target, expectedUrl) {
  ...
  async function clickSelector(selector) {
    for (let attempt = 0; attempt < 8; attempt += 1) {
      const rect = await evaluate(async (value) => {
        const element = document.querySelector(value);
        if (!element || element.disabled) return null;
        element.scrollIntoView({ block: 'center', inline: 'center' });
        await new Promise((resolve) => setTimeout(resolve, 150));
        if (!element.isConnected || element !== document.querySelector(value) || element.disabled) return null;
        const box = element.getBoundingClientRect();
        return box.width > 0 && box.height > 0 ? { x: box.left + box.width / 2, y: box.top + box.height / 2 } : null;
      }, selector);
      if (rect) { await clickRect(rect); return; }
    }
    fail('E2E_SIDEPANEL_CONTROL_MISSING', 'The requested authentic side-panel control did not remain stable long enough to click.', { selector });
  }
  ...
  async function clickButton(text, { cardIndex = null, containsText = null } = {}) {
    for (let attempt = 0; attempt < 8; attempt += 1) {
      const rect = await evaluate(async ({ text, cardIndex, containsText }) => {
    ...
    fail('E2E_SIDEPANEL_CONTROL_MISSING', 'The requested authentic side-panel button did not remain stable long enough to click.', { text, cardIndex, containsText });
  }
  ...
  async function captureScreenshot(filePath, { fullPage = true, selector = null } = {}) {
    const absolutePath = path.resolve(filePath);
    await evaluate((selector) => new Promise((resolve) => {
      if (selector) {
        document.querySelector(selector)?.scrollIntoView({ block: 'start' });
        globalThis.scrollBy(0, -80);
      }
...
async function main(options) {
  ...
  try {
    ...
    report.config.tempDebuggerGrant = false;
    report.checks.push({
    ...
    const mission = await waitFor('side-panel mission objective and exact member binding', async () => {
      const response = await sendUi(extensionPage, fixturePage, 'UI_GET_STATE', {}, fixture.origin);
      if (response?.ok !== true) {
        throw new Error(`${response?.error?.code ?? 'UI_GET_STATE_FAILED'}: ${response?.error?.message ?? 'No mission state returned.'}`);
      }
      ...
      if (!candidate || !member) {
        throw new Error(`MISSION_PENDING: ${JSON.stringify((response.state?.missions ?? []).map((entry) => ({ missionId: entry?.missionId, objective: entry?.objective, phase: entry?.phase, members: entry?.members })))}`);
      }
    ...
    const completedMission = await waitFor('mission completion through authentic side panel', async () => {
      const response = await sendUi(extensionPage, fixturePage, 'UI_GET_STATE', {}, fixture.origin);
      if (response?.ok !== true) {
        throw new Error(`${response?.error?.code ?? 'UI_GET_STATE_FAILED'}: ${response?.error?.message ?? 'No completed mission state returned.'}`);
      }
      const currentMission = response?.state?.missions?.find((entry) => entry?.missionId === mission.missionId);
      if (currentMission?.phase !== 'completed') {
        const ui = await extensionPage.evaluate(() => ({
          toast: document.querySelector('#toast')?.textContent ?? '',
          completeButton: document.querySelector('[data-action="complete-mission"]')?.textContent ?? '',
        }));
        throw new Error(`MISSION_PENDING: ${JSON.stringify({ phase: currentMission?.phase ?? null, revision: currentMission?.revision ?? null, ui })}`);
      }
...
export {
  ...
  pageRuntimeState,
  openTrustedSidePanel,
  fixtureFormArguments,
};
