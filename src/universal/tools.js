...
export function generateWebMcpToolDescriptors(input, options = {}) {
  ...
  if (normalizedOptions.includePageRead) {
    ...
    const scrollTargets = [null, ...snapshot.elementRefs.filter((element) => element.attributes?.['toolbraid-scrollable']).slice(0, 8)];
    for (const element of scrollTargets) {
      if (descriptors.length >= normalizedOptions.maxTools) break;
      const item = element ?? { ref: null, name: 'page' };
      const descriptor = createDescriptor(snapshot, item, element ? 'element' : 'page', {
        classification: ACTION_CLASSES.READ, risk: 'read-only', requiresApproval: false,
        evidence: [{ source: 'browser-viewport', code: 'VIEWPORT_SCROLL' }],
      }, normalizedOptions, 1);
      Object.assign(descriptor, {
        name: `${slug(normalizedOptions.namePrefix, 'universal')}_scroll_${slug(element?.ref, 'page')}`,
        title: `Scroll ${element?.name || element?.ref || 'page'}`,
        description: 'Scroll this exact viewport to read more content. Changes only the viewport; never clicks, types, submits or navigates. Lazy content may load. Returns measured before/after positions and refreshed page text; list tools again after content changes.',
        inputSchema: { type: 'object', additionalProperties: false, required: ['direction'], properties: {
          direction: { type: 'string', enum: ['up', 'down', 'left', 'right', 'top', 'bottom'] },
          pixels: { type: 'integer', minimum: 1, maximum: 4000, description: 'Distance for directional scrolling; defaults to 80% of the viewport. Omit for top/bottom.' },
        } },
        effect: { classification: ACTION_CLASSES.READ, operation: 'scroll', summary: 'Scroll the exact browser viewport and observe its position.', externalStateChange: false, requiresApproval: false },
        annotations: { readOnlyHint: true, idempotentHint: false, untrustedContentHint: true },
      });
      validateToolDescriptor(descriptor);
