...
import { uniqueControl, verifiedActionDescriptor } from './action.js';
import { X_AUTHORING_ACTIONS, generateXAuthoringTools, isXContentRoute, readXContext, verifyXPublication } from './x-authoring.js';

const DEFAULT_HOSTS = Object.freeze(['x.com', 'www.x.com', 'twitter.com', 'www.twitter.com']);
const DIRECT_ACTIONS = new Set([
  'prepare_x_reply', 'prepare_x_post', 'publish_x_post', 'like_x_post', 'repost_x_post',
  'open_x_repost_menu', 'open_x_quote_composer',
]);

export function isXDirectAction(tool) {
  if (tool?.classification !== 'mutate' || (!DIRECT_ACTIONS.has(tool.name) && !X_AUTHORING_ACTIONS.has(tool.name))
    || tool.provenance?.source !== 'toolbraid.verified-adapter'
    || tool.provenance?.adapterId !== 'x-post') return false;
  try {
    const url = new URL(tool.provenance.url);
    return url.protocol === 'https:' && DEFAULT_HOSTS.includes(url.hostname)
      && url.origin === tool.provenance.origin;
  } catch { return false; }
}
...
export const X_POSTCONDITION_IDS = Object.freeze({
  ...
  unlike: 'x.post.unlike.v1',
  publish: 'x.post.publish.v1',
});
...
function descriptor(snapshot, adapterVersion, {
  ...
  const tool = {
    ...
    target: {
      ...
      type: target ? targetType : 'verified-adapter',
      targetFingerprint: target ? elementFingerprint(target) : null,
      ...(target ? { binding: { role: target.role ?? (targetType === 'link' ? 'link' : null), name: target.name ?? target.text ?? '', type: target.type ?? null, formRef: target.formRef ?? null } } : {}),
    },
    elementRef: target?.ref ?? null,
...
function postScope(snapshot) {
  ...
  const article = permalinkArticle(snapshot, elements)
    ?? (!/\/status\//.test(pageUrl(snapshot)?.pathname ?? '') ? snapshot.elementRefs.find((element) => dataTestId(element) === 'tweet') : null);
...
function controlsForPost(snapshot) {
  ...
  if (!scope) return snapshot.elementRefs.some((element) => element.tagName === 'article') ? [] : snapshot.accessibleControls;
  return snapshot.accessibleControls.filter((control) => {
    ...
    while (parentRef && !visited.has(parentRef)) {
      ...
      if (parent.ref === scope.article.ref) return true;
      if (normalizedText(parent.tagName).toLowerCase() === 'article') return false;
      parentRef = parent.parentRef;
    }
    return false;
  });
...
function findRepostMenuControl(snapshot) {
  const matches = controlsForPost(snapshot).filter((control) => control.disabled !== true
    && normalizedText(control.role).toLowerCase() === 'button'
    && (['retweet', 'repost', 'unretweet', 'unrepost'].includes(dataTestId(control))
      || /^(?:repost|retweet|repostare|repostează|reposteaza)$/iu.test(normalizedText(control.name))));
  return matches.length === 1 ? matches[0] : null;
}

function findQuoteControl(snapshot) {
  return uniqueControl(snapshot, (control) => availableControl(control)
    && normalizedText(control.role).toLowerCase() === 'menuitem'
    && /^(?:quote|quote post|quote tweet|citează|citeaza|citare)$/iu.test(normalizedText(control.name)));
}
...
function findPostEditor(snapshot) {
  return uniqueControl(snapshot, (control) => {
    if (!availableControl(control)) return false;
    const role = normalizedText(control.role).toLowerCase();
    const type = normalizedText(control.type).toLowerCase();
    if (role !== 'textbox' && !['text', 'textarea'].includes(type)) return false;
    return /^tweettextarea(?:_\d+)?$/.test(dataTestId(control))
      || /^(?:what is happening\??|what's happening\??|post text|text postare)$/iu.test(normalizedText(control.name));
  });
}

function findPublishControl(snapshot) {
  if (snapshot.metadata.xContent?.uploadBusy === true) return null;
  return uniqueControl(snapshot, (control) => {
    if (!availableControl(control) || normalizedText(control.role).toLowerCase() !== 'button') return false;
    const testId = dataTestId(control);
    return ['tweetbutton', 'tweetbuttoninline'].includes(testId)
      || /^(?:post|tweet|postează|posteaza)$/iu.test(normalizedText(control.name));
  });
}
...
function verifyXPostcondition({ tool, contract: suppliedContract, beforeSnapshot, afterSnapshot }, hosts, allowFixture, adapterVersion) {
  ...
  const expectedIds = {
    ...
    unlike_x_post: X_POSTCONDITION_IDS.unlike,
  ...
  if (tool.name === 'like_x_post' || tool.name === 'unlike_x_post') {
    ...
    const removing = tool.name === 'unlike_x_post';
    if ((removing ? before.active.length !== 1 || before.available.length !== 0 : before.available.length !== 1 || before.active.length !== 0) || !target) {
      return postconditionResult('unverified', 'X_LIKE_PRECONDITION_NOT_CONFIRMED', afterSnapshot, evidence);
    }
    if (removing ? after.available.length !== 1 || after.active.length !== 0 : after.active.length !== 1 || after.available.length !== 0) {
      return postconditionResult('unverified', 'X_LIKE_STATE_NOT_CONFIRMED', afterSnapshot, evidence);
    }
    return postconditionResult('verified-success', removing ? 'X_UNLIKE_STATE_CONFIRMED' : 'X_LIKE_STATE_CONFIRMED', afterSnapshot, evidence);
  }
...
export function extractXPost(snapshot) {
  const currentId = pageUrl(snapshot)?.pathname.match(/^\/[A-Za-z0-9_]+\/status\/(\d+)$/)?.[1];
  const observed = snapshot.metadata.xContent?.posts?.find((post) => post.id === currentId);
  if (observed) return Object.freeze({ ...structuredClone(observed), type: 'x-post', handle: observed.author,
    pageFingerprint: snapshot.pageFingerprint, provenance: 'toolbraid.verified-adapter/x-post', untrustedContent: true });
  if (currentId && snapshot.metadata.xContent?.posts?.length) return Object.freeze({ type: 'x-post', url: snapshot.metadata.url,
    text: null, available: false, reason: 'The exact parent post is not in the rendered snapshot. Use read_x_conversation or scroll to its position.',
  ...
  return Object.freeze({
    ...
    url: explicit?.url ?? (currentId ? `${currentUrl.origin}${currentUrl.pathname}` : statusLink?.href ?? snapshot.metadata.url),
...
export function createXPostAdapter({ hosts = DEFAULT_HOSTS, allowFixture = false, version = '1' } = {}) {
  ...
  return Object.freeze({
    ...
    matches(snapshot) {
      const url = pageUrl(snapshot);
      if (!url) return false;
      if (allowedHosts.has(url.hostname.toLowerCase())) return isXContentRoute(url);
      return allowFixture && url.pathname.startsWith('/x-post');
    },
    generateTools(snapshot) {
      const onPost = /\/status\//.test(pageUrl(snapshot)?.pathname ?? '') || snapshot.metadata.pageType === 'x-post';
      const tools = onPost ? [descriptor(snapshot, version, {
        name: 'read_x_post',
        title: 'Read X post',
      ...
      tools.push(...generateXAuthoringTools(snapshot, version, descriptor));
      if (onPost && !findReplyEditor(snapshot)) {
        const replies = controlsForPost(snapshot).filter((control) => control.disabled !== true && control.role === 'button'
          && (dataTestId(control) === 'reply' || /^(?:reply|răspuns|raspuns)$/iu.test(normalizedText(control.name))));
        if (replies.length === 1) tools.push(verifiedActionDescriptor(snapshot, { adapterId: 'x-post', adapterVersion: version,
          name: 'open_x_reply_composer', title: 'Open X reply composer', target: replies[0], risk: 'account-content',
          description: 'Open a reply to the exact current post. Read the composer and target context before preparing text.', summary: 'Open the exact X reply editor without sending.' }));
      }
      // The adapter stages text only when X exposes an actual reply editor.
      // A closed composer has no editor to fill; a refreshed snapshot exposes
      // its exact text target once the composer is open.
      const replyEditor = onPost ? findReplyEditor(snapshot) : null;
      if (replyEditor) tools.push(descriptor(snapshot, version, {
        ...
        inputSchema: {
          type: 'object',
          properties: { text: { type: 'string', minLength: 1, maxLength: 10_000, description: 'Text to place in the open X composer without publishing yet.' } },
          required: ['text'],
      ...
      if (!onPost) {
        const postEditor = findPostEditor(snapshot);
        if (postEditor) tools.push(verifiedActionDescriptor(snapshot, {
          adapterId: 'x-post', adapterVersion: version, name: 'prepare_x_post',
          title: 'Prepare X post', description: 'Set text in the exact visible X post composer.',
          classification: 'mutate', risk: 'account-content', target: postEditor,
          inputSchema: { type: 'object', properties: { text: { type: 'string', minLength: 1, maxLength: 10_000 } }, required: ['text'], additionalProperties: false },
          summary: 'Set text in the visible X post composer.',
        }));
      }
      const publish = findPublishControl(snapshot);
      if (publish) tools.push(descriptor(snapshot, version, {
        name: 'publish_x_post', risk: 'account-content', classification: 'mutate',
        title: 'Publish X composer', description: 'Publish the prepared post, reply or quote using the exact visible X publish control.',
        target: publish, summary: 'Publish the currently prepared X composer.', postcondition: postconditionContract(X_POSTCONDITION_IDS.publish, version),
      }));
      const unlike = controlsForPost(snapshot).filter((control) => control.disabled !== true && likeControlState(control) === 'active');
      if (unlike.length === 1) tools.push(descriptor(snapshot, version, { name: 'unlike_x_post', title: 'Remove X like', description: 'Remove the existing like from the exact current post. Use only when explicitly requested; never as a substitute for liking.',
        classification: 'mutate', risk: 'account-content', target: unlike[0], summary: 'Remove the existing X like.', postcondition: postconditionContract(X_POSTCONDITION_IDS.unlike, version) }));
      for (const [id, name, title] of [['bookmark', 'bookmark_x_post', 'Bookmark X post'], ['removebookmark', 'remove_x_bookmark', 'Remove X bookmark']]) {
        const controls = controlsForPost(snapshot).filter((control) => control.disabled !== true && dataTestId(control) === id);
        if (controls.length === 1) tools.push(verifiedActionDescriptor(snapshot, { adapterId: 'x-post', adapterVersion: version, name, title, target: controls[0], risk: 'account-content', description: `${title} using the exact current post control. Read its state afterwards.`, summary: title }));
      }
      const like = findLikeControl(snapshot);
      if (like) tools.push(descriptor(snapshot, version, {
        name: 'like_x_post',
        title: 'Like X post',
        description: 'Like the exact visible post through the local owner policy.',
        classification: 'mutate',
        risk: 'account-content',
        target: like,
      ...
      // confirmation menu item is live. Opening the menu is a separate action,
      // never a claim that a repost has already completed.
      const repostMenu = onPost ? findRepostMenuControl(snapshot) : null;
      if (repostMenu) tools.push(verifiedActionDescriptor(snapshot, {
        adapterId: 'x-post', adapterVersion: version, name: 'open_x_repost_menu', risk: 'account-content',
        title: 'Open X repost menu', description: 'Open the exact post menu to choose Repost or Quote. This does not publish anything.',
        target: repostMenu, summary: 'Open the current X post repost and quote menu.',
      }));
      const quote = onPost ? findQuoteControl(snapshot) : null;
      if (quote) tools.push(verifiedActionDescriptor(snapshot, {
        adapterId: 'x-post', adapterVersion: version, name: 'open_x_quote_composer', risk: 'account-content',
        title: 'Quote X post', description: 'Open a quote composer for the current X post. Add text and use the publish tool separately.',
        target: quote, summary: 'Open the quote composer for the current X post without publishing yet.',
      }));
      const repost = findRepostConfirmation(snapshot);
      if (repost) tools.push(descriptor(snapshot, version, {
        name: 'repost_x_post',
        title: 'Repost X post',
        description: 'Repost the exact visible post through the local owner policy.',
        classification: 'mutate',
        risk: 'account-content',
        target: repost,
    ...
    verifyPostcondition(context = {}) {
      return verifyXPublication(context) ?? verifyXPostcondition(context, allowedHosts, allowFixture, version);
    },
    executeRead(tool, snapshot) {
      if (tool.name === 'read_x_post') return extractXPost(snapshot);
      const result = readXContext(tool.name, snapshot);
      if (!result) throw new Error(`Unsupported X read tool: ${tool.name}`);
      return result;
    },
  });
}
