...
import { createOwnerAutonomyPolicy } from '../policy/owner-autonomy.js';
...
export function createUniversalSessionRuntime({
  ...
  now = () => new Date(),
  authorizationPolicy = createOwnerAutonomyPolicy(),
  ownerAuthorizationPolicy = authorizationPolicy,
} = {}) {
  ...
  if (!authorizationPolicy || typeof authorizationPolicy.decide !== 'function') throw new TypeError('authorizationPolicy with decide() is required.');
  if (!ownerAuthorizationPolicy || typeof ownerAuthorizationPolicy.decide !== 'function') throw new TypeError('ownerAuthorizationPolicy with decide() is required.');
  ...
  async function ingestSnapshot({ tabId, frameId = 0, sessionId, snapshot: rawSnapshot, mediaAssets = [] }) {
    ...
    const generic = generateWebMcpToolDescriptors(snapshot, { includePageRead: true, maxTools: 120 })
      .filter((descriptor) => descriptor.effect?.operation === 'scroll' || !descriptor.target?.ref || !verifiedTargets.has(descriptor.target.ref))
      .map(ownDescriptor);
    const viewportTools = generic.filter((tool) => tool.sourceType === 'page' || tool.effect?.operation === 'scroll');
    const combined = [...viewportTools, ...packDescriptors, ...verified, ...generic.filter((tool) => !viewportTools.includes(tool))];
    const accepted = [];
    ...
    const nativeUnavailable = registration?.ok === false && registration.error?.code === 'WEBMCP_UNSUPPORTED';
    if (registration?.ok === false && !nativeUnavailable) {
      throw sessionError('TOOL_REGISTRATION_FAILED', 'Generated WebMCP tool registration failed.', { registration });
    }
    state.toolTransport = nativeUnavailable ? 'extension-mcp' : 'native-webmcp';
  ...
  async function executeToolWithPolicy({ tabId, frameId = 0, sessionId, name, input = {} }, activeAuthorizationPolicy) {
    ...
    if (descriptor.classification === 'read') {
      if (descriptor.effect?.operation === 'scroll' && descriptor.provenance?.source === 'toolbraid.universal') {
        const preparedAction = prepareAction({ snapshot: state.snapshot, descriptor, input, includeDescriptor: true });
        const result = await executePageAction({ tabId, frameId, sessionId, preparedAction, approved: false, mode: 'read',
          beforeDispatch: () => append(state, 'viewport.scrolling', { name, pageFingerprint: preparedAction.pageFingerprint, arguments: input }),
        });
        await append(state, 'viewport.scrolled', { name, before: result.before, after: result.after, moved: result.moved });
        return result;
      }
    ...
    const policyDecision = await activeAuthorizationPolicy.decide(Object.freeze({
      classification: prepared.classification,
      origin: descriptor.provenance.origin,
      capability: `page.action.${prepared.classification}`,
      descriptor: clone(descriptor),
      preparedAction: clone(prepared),
    }));
    if (!plainObject(policyDecision) || !['auto', 'manual', 'deny'].includes(policyDecision.decision)) {
      state.pending.delete(prepared.actionId);
      throw sessionError('AUTHORIZATION_POLICY_INVALID', 'Authorization policy returned an invalid decision.');
    }
    if (policyDecision.decision === 'deny') {
      state.pending.delete(prepared.actionId);
      await append(state, 'action.denied', { actionId: prepared.actionId, reason: boundedString(policyDecision.reason, 128) ?? 'policy-denied' });
      return Object.freeze({ status: 'denied', preparedAction: clone(prepared), reason: boundedString(policyDecision.reason, 128) ?? 'policy-denied' });
    }
    if (policyDecision.decision === 'auto') {
      const envelope = await authorizePending(state, prepared.actionId, undefined, {
        source: 'owner-policy',
        policyDecision,
      });
      return Object.freeze({ status: 'auto-authorized', preparedAction: clone(prepared), approval: clone(envelope) });
    }
    return Object.freeze({ status: 'approval-required', preparedAction: clone(prepared) });
  }

  async function executeTool(request) {
    return executeToolWithPolicy(request, authorizationPolicy);
  }

  async function authorizePending(state, actionId, ttlMs, { source = 'human', policyDecision = null } = {}) {
    ...
    state.approvals.set(actionId, { envelope, context, source, policyDecision: clone(policyDecision) });
    try {
      await append(state, 'approval.created', {
        actionId,
        fingerprint: envelope.fingerprint,
        expiresAt: envelope.expiresAt,
        authorizationSource: source,
        ...(policyDecision?.leaseId ? { leaseId: boundedString(policyDecision.leaseId, 128) } : {}),
      });
    } catch (error) {
  ...
  async function approve({ tabId, frameId = 0, sessionId, actionId, ttlMs }) {
    ...
    return authorizePending(state, actionId, ttlMs);
  }
  ...
  async function executeApproved({ tabId, frameId = 0, sessionId, actionId, snapshot: refreshedSnapshot }) {
    ...
    try {
      ...
      await append(state, 'approval.claimed', {
        actionId,
        nonce: claim.nonce,
        fingerprint: claim.fingerprint,
        authorizationSource: approval.source ?? 'human',
      });
    } catch (error) {
  ...
  async function executeOwnerAction({ tabId, frameId = 0, sessionId, name, input = {}, snapshot }) {
    const prepared = await executeToolWithPolicy(
      { tabId, frameId, sessionId, name, input },
      ownerAuthorizationPolicy,
    );
    if (prepared?.status !== 'auto-authorized') return prepared;
