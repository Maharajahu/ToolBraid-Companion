...
const WINDOWS_PIPE_PATTERN = /^\\\\\.\\pipe\\toolbraid-(?:personal-)?mcp-[a-f0-9-]{8,80}$/;
...
export function validateBridgeConfig(value) {
  const allowedOrigins = plainObject(value) ? (value.allowedOrigins ?? [value.allowedOrigin]) : [];
  if (!plainObject(value)
    || value.version !== BRIDGE_CONFIG_VERSION
    || typeof value.token !== 'string' || !TOKEN_PATTERN.test(value.token)
    || typeof value.pipe !== 'string'
    || !Array.isArray(allowedOrigins) || allowedOrigins.length < 1 || allowedOrigins.length > 4
    || allowedOrigins.some((origin) => typeof origin !== 'string' || !EXTENSION_ORIGIN_PATTERN.test(origin))) {
